...
2. **Async execution: checkpointed status machine now, Inngest before prod go-live.** `lib/feedback/pipeline.ts` checkpoints every step onto the Feedback row (`ticket` json → `jiraKey` → `screenshotAttachedAt` → `notifiedAt`) and skips completed steps, so `POST /api/feedback/:id/retry` (SITE_ADMIN) resumes from the failed step with zero duplicate Jira tickets. The route currently fires the pipeline fire-and-forget — correct on the persistent `tsx watch` dev server, but on Vercel serverless the function freezes after the response, so **Inngest (or equivalent) is a required step before production**; the pipeline steps are already shaped as its `step.run()` blocks.
