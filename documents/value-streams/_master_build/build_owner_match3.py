...
JUNIOR = re.compile(r"associate|technician|assistant|trainee|junior|\bclerk\b|coordinator|representative|\brep\b|\banalyst\b|intern|processor|\bagent\b|data entry|operator|\bsteward\b")
LVL = {0: "Junior IC", 1: "Specialist", 2: "Manager", 3: "Senior/Exec"}
def rank(role, level=None):
    n = role.lower(); lv = (level or "").lower()
    if lv in ("executive", "leadership") or re.search(r"\bchief\b|head of |\bdirector\b|\bvp\b|president|general counsel|appointed actuary", n): return 3
    if lv == "manager" or re.search(r"\bmanager\b|\bhead\b|principal|\blead\b|superintendent", n): return 2
    if JUNIOR.search(n): return 0
    return 1  # qualified specialist / professional IC (actuary, underwriter, adjuster, engineer, accountant, counsel, officer...)

LEAD = re.compile(r"\bdepartment\b|enterprise|operating budget|\bbudget\b|strateg|operating model|governance|set (the )?standards?|\bcharter\b|policy framework|capital allocation|\broadmap\b|appetite framework|target operating|annual (operating )?plan|define (the )?(approach|framework|policy|strategy)")
GOVERN = re.compile(r"operating plan|business plan|\bplan\b|planning|resource|staffing|capacity plan|headcount|oversight|prioriti|allocat|manage the|coordinate the program|workforce")
...
def altitude(name):
    ...
    if LEAD.search(n): return "lead", 3       # department/budget/strategy/governance -> head/senior
    if GOVERN.search(n): return "govern", 2   # plan/resource/coordinate -> manager
    return "execute", 1                        # hands-on work -> qualified specialist (never a junior)

def pick(name, deliverable, l4, roles):
    alt, target = altitude(name)
    ttok = toks(name + " " + deliverable + " " + l4)
    has_senior = any(rank(r["role"], r.get("level")) >= 1 for r in roles)
    scored = []
    for r in roles:
        rk = rank(r["role"], r.get("level"))
        ov = len(ttok & toks(r["role"] + " " + " ".join(r.get("does", [])) + " " + (r.get("family") or "")))
        d = abs(rk - target)
        fit = 5 if d == 0 else (1 if d == 1 else -6)
        if alt == "execute" and rk >= 3: fit -= 6        # no exec leading hands-on work
        if rk == 0 and has_senior: fit -= 12             # HARD: never a junior as lead when a qualified role exists
        scored.append((ov * 2 + fit, rk, ov, r["role"]))
    if not scored: return "", [], "Specialist"
    scored.sort(key=lambda x: (-x[0], -x[2]))
    owner = scored[0][3]; owner_lvl = LVL[scored[0][1]]
    docand = sorted([s for s in scored if s[3] != owner], key=lambda x: (-(x[2]), x[1]))
    participants = [s[3] for s in docand[:2]]
    return owner, participants, owner_lvl
...
COLS = ["L1 Segment","L2 Division","L3 Process","L4 Sub-Process","L5 Task","isTask","Owner / Lead Role",
        "Roles (Participants)","Primary App / SoR","Supporting Apps","Deliverable","Checklist","Automatability","Altitude","Owner Level"]
rows = []; altc = Counter()
for k, area in tba.items():
    ...
    for g in area.get("l4groups", []):
        ...
        for t in g.get("tasks", []):
            ...
            owner, pcs, olvl = pick(name, deliv, l4, roles)
            ...
            rows.append([l1, l2, l3, l4, name, "Y", owner, "; ".join(pcs), primary, "; ".join(supp),
                         deliv, " | ".join(t.get("checklist", [])), t.get("automatability", ""), alt, olvl])
...
ws.freeze_panes = "E2"
for i, w in enumerate([13,22,26,28,40,5,24,30,22,28,34,50,12,9,12], 1):
    ws.column_dimensions[get_column_letter(i)].width = w
ws.auto_filter.ref = f"A1:{get_column_letter(len(COLS))}{len(rows)+1}"
out = "documents/value-streams/ABC-Insurance-Operating-Model-MASTER-v10-FIXED.xlsx"
wb.save(out)
# audit: any junior owners left?
jr = sum(1 for r in rows if r[14] == "Junior IC")
from collections import Counter as C2
lvlc = C2(r[14] for r in rows)
print("SAVED", out, "| tasks", len(rows), "| altitude", dict(altc))
print("owner levels:", dict(lvlc), "| JUNIOR-as-lead remaining:", jr)
def show(sub):
    for r in rows:
        if sub.lower() in r[4].lower():
            print(f"  [{r[1]}] {r[4]}\n    owner: {r[6]} ({r[14]}) | participants: {r[7]}"); return
show("department operating budget"); show("Extract in-force and claims data")
show("go/no-go review and record launch"); show("Capture structured loss facts")
