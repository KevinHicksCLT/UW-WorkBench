...
// ── Smart back ──────────────────────────────────────────────────────────────
// The one back behavior for the whole app (ui/BackButton). Back means, in
// order:
//   1. Pop the deepest IN-PAGE drill level — drilled surfaces register a
//      handler via useBackHandler.
//   2. History back — React Router stamps an entry index on `history.state`
//      (`idx`); idx > 0 means an in-app entry is behind us.
