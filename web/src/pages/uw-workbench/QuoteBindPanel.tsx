...
// correlationId. Bind targets come from the company's Application catalog
// (GET /uw/catalog), falling back to the submission's own PAS bindings.
...
type Catalog = { applications: { id: string; name: string; kind: string }[] };
...
export function QuoteBindPanel({
  ...
  // PAS bind target: the company's registered Application catalog, with the
  // submission's own PAS bindings as fallback.
  const { data: catalog } = useApi<Catalog>('/uw/catalog');
  const fromBindings = submission.pasBindings.map((b) => ({
    id: b.applicationId,
    name: b.application.name,
  }));
  const appOptions = catalog?.applications.length
    ? catalog.applications.map((a) => ({ id: a.id, name: a.name }))
    : fromBindings;
  ...
  async function executeBind() {
    ...
    setBusy(true);
    try {
      const bind = (await api.post('/uw/bind', {
        quoteId: quote,
