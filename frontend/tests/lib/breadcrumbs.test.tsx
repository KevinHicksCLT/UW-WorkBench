...
describe('trail building', () => {
  ...
  it('appends Home + tab when a tab base is reached by link navigation', () => {
    renderTrail('/');
    act(() => navigateFn('/roles'));
    expect(trail()).toBe('Home@/ | Roles@/roles');
  });
  ...
  it('keeps walking across tab boundaries (cross-tab links append)', () => {
    ...
    expect(trail()).toBe('Home@/ | Roles@/roles | Standards@/standards');
  });
  ...
  it('navigating to a URL already in the trail truncates back to it and keeps the fresh search', () => {
    ...
    expect(trail()).toBe('Home@/ | Roles@/roles?f=claims');
  });
...
describe('resetToTab', () => {
  it('re-roots the trail to [Home, tab] for a top-nav click', () => {
    renderTrail('/');
    act(() => navigateFn('/roles'));
    act(() => navigateFn('/standards'));
    act(() => resetFn('/standards'));
    expect(trail()).toBe('Home@/ | Standards@/standards');
  });
  ...
  it('clears the trail for the wordmark (/) and ignores non-tab URLs', () => {
    ...
    expect(trail()).toBe('Home@/ | Standards@/standards'); // unchanged by the no-op reset
  });
});

describe('useRegisterCrumb', () => {
  it('appends the page crumb after the owning tab on a deep link (seeded root)', () => {
    renderTrail('/roles/r42', <RegisterCrumb label="Claims Manager" />);
    expect(trail()).toBe('Home@/ | Roles@/roles | Claims Manager@/roles/r42');
  });

  it('collapses to just Home when the page belongs directly under it', () => {
