...
const nodeName = (n: Node): string =>
  (n.data as { name?: string; label?: string }).name ?? (n.data as { label?: string }).label ?? '';
...
export function useMapDragDrop({
  ...
  const currentParentRaw = useCallback(
    (node: Node): string | null => {
      switch (node.type) {
        case 'divisionNode':
          return domainIdByCat.get((node.data as DivisionNodeData).category) ?? null;
        case 'valueStreamNode':
          return focusedDivisionId;
        case 'stepNode':
          return focusedVsId;
        case 'subStepNode':
          return focusedStepId;
        default:
          return null; // domain / company
      }
    },
  ...
  const isVisibleDescendant = useCallback(
    (targetRaw: string, draggedRaw: string): boolean => {
      const inChainBelowDivision =
        valueStreams.some((v) => v.id === targetRaw) ||
        steps.some((s) => s.id === targetRaw) ||
        (focusedStep?.subSteps.some((s) => s.id === targetRaw) ?? false);
      const inChainBelowVs =
        steps.some((s) => s.id === targetRaw) ||
        (focusedStep?.subSteps.some((s) => s.id === targetRaw) ?? false);
      if (draggedRaw === focusedDivisionId && inChainBelowDivision) return true;
      if (draggedRaw === focusedVsId && inChainBelowVs) return true;
      if (
        draggedRaw === focusedStepId &&
        (focusedStep?.subSteps.some((s) => s.id === targetRaw) ?? false)
      )
        return true;
  ...
  const scheduleHoverDrill = useCallback(
    (canvasId: string | null, type: string | null) => {
      if (
        !canvasId ||
        !type ||
        !['coreNode', 'divisionNode', 'valueStreamNode', 'stepNode'].includes(type)
      ) {
        clearHoverDrill();
        return;
      }
      ...
      hoverDrillRef.current = {
        ...
        timer: window.setTimeout(() => {
          if (type === 'coreNode') {
            const c = canvasId.slice(5);
            if (selectedDomain !== c) onDomainClick(c as Category);
          } else if (type === 'divisionNode') {
            if (focusedDivisionId !== canvasId) onDivisionClick(canvasId);
          } else if (type === 'valueStreamNode') {
            const raw = canvasId.slice(3);
            if (focusedVsId !== raw) onVsClick(raw);
          } else if (type === 'stepNode') {
            const raw = canvasId.slice(5);
            if (focusedStepId !== raw) onStepClick(raw);
          }
        }, HOVER_DRILL_MS),
      };
    },
  ...
  const computeGap = useCallback(
    (d: DragState): { parent: string; index: number; type: string } | null => {
      type RowRect = {
        id: string;
      ...
      for (const type of ROW_TYPES) {
        ...
        const rects: RowRect[] = rows.map((n) => ({
          id: n.id,
          r: nodeScreenRect(n),
          raw: rawNodeId(n),
        }));
        ...
        const dist = horizontal
          ? Math.abs(d.py - (r0.y + r0.h / 2))
          : Math.abs(d.px - (r0.x + r0.w / 2));
      ...
      const main = (r: { x: number; y: number; w: number; h: number }) =>
        best!.horizontal ? r.x + r.w / 2 : r.y + r.h / 2;
  ...
  const containerAtCursor = useCallback(
    (d: DragState): { canvasId: string; type: string } | null => {
      for (const n of nodes) {
        if (n.id === d.canvasId) continue;
        if (!['coreNode', 'divisionNode', 'valueStreamNode', 'stepNode'].includes(n.type ?? ''))
          continue;
        const r = nodeScreenRect(n);
        if (d.px >= r.x && d.px <= r.x + r.w && d.py >= r.y && d.py <= r.y + r.h)
  ...
  const onStagePointerDown = useCallback(
    (e: ReactPointerEvent) => {
      ...
      dragRef.current = {
        ...
        rawId,
        type: hit.type,
        level: TYPE_LEVEL[hit.type] ?? 0,
        name: nodeName(node),
        cat: (node.data as { category?: string }).category ?? selectedDomain ?? '',
        originParent: origin,
