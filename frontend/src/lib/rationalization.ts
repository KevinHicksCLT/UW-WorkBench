...
export const CAPDAN_META: Record<Capdan, { label: string; chip: string; dot: string }> = {
  ...
  Different: { label: 'Different', chip: 'bg-[#eff6ff] text-[#1d4ed8] border-[#bfdbfe]', dot: '#2563eb' },
  Relocate: { label: 'Relocate', chip: 'bg-[#f5f3ff] text-[#6d28d9] border-[#ddd6fe]', dot: '#7c3aed' },
  Eliminate: { label: 'Eliminate', chip: 'bg-[#fff1f2] text-[#be123c] border-[#fecdd3]', dot: '#e11d48' },
};
...
export const STATUS_ORDER = ['Identified', 'In Analysis', 'Normalized', 'In Migration', 'Migrated', 'Retired'];
...
// Group one cell's findings (app × layer) into category tags.
export function categoryTags(findings: Finding[], layer: Layer, appId?: string): CategoryTag[] {
  ...
  for (const f of findings) {
    ...
    if (existing) { existing.count++; existing.findings.push(f); }
    else byKey.set(key, { appId: f.appId, layer, category: cat, capdan: f.capdan, targetLayer: f.targetLayer, count: 1, findings: [f] });
  }
