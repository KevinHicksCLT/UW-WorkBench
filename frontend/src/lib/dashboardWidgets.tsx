...
function Tile({
  ...
  if (to) {
    return (
      <UICard
        ...
        variant="elevated"
        to={to}
        className="p-4 block h-full text-center transition-colors hover:border-[#4f46e5] group"
      >
        {body}
      </UICard>
    );
  ...
  return (
    <UICard variant="elevated" className="p-4 h-full text-center">
      {body}
    </UICard>
  );
}
      ...
      {groups.map((g) => {
        ...
        return (
          <div key={g.key} className="flex items-center gap-3">
            ...
            <div className="w-10 text-right text-xs text-[#171717] tnum flex-shrink-0">
              {g.count}
            </div>
          </div>
        );
  ...
  return (
    <UICard variant="elevated" className="p-5 h-full">
      <div className="flex items-center justify-between mb-4">
        {to ? (
          <Link to={to} className="text-sm font-semibold text-[#171717] hover:text-[#4f46e5]">
            {title}
          </Link>
        ) : (
        ...
        {to && (
          <Link to={to} className="text-xs text-[#525252] hover:text-[#171717]">
            {toLabel ?? 'View'} →
          </Link>
        )}
      </div>
...
const HEALTH_COLOR = (k: string) =>
  ({ Green: '#16a34a', Amber: '#d97706', Red: '#dc2626' })[k] ?? '#94a3b8';
const SEVERITY_COLOR = (k: string) =>
  ({ Critical: '#dc2626', High: '#ea580c', Medium: '#d97706', Low: '#65a30d' })[k] ?? '#94a3b8';
...
const tile = (
  ...
  render: (d) => (
    <Tile
      label={wt(d, id, label)}
      value={d.totals[totalKey] ?? 0}
      hint={opts.hint?.(d.totals)}
      to={opts.to}
    />
  ),
});
...
const txn = (d: Dashboard, body: (t: TransformationData) => ReactNode): ReactNode =>
  d.transformation ? (
    body(d.transformation)
  ) : (
    <EmptyState baseClassName="text-sm text-[#a3a3a3]" message="No transformation data yet." />
  );

export const WIDGET_CATALOG: Widget[] = [
  // ── Transformation command center (D1) ──
  {
    id: 'card:portfolioRollup',
    title: 'Transformation Portfolio',
    kind: 'wide',
    source: SRC.initiatives,
  ...
  {
    id: 'card:raidByProgram',
    title: 'RAID by program',
    kind: 'wide',
    source: SRC.risks,
  ...
  {
    id: 'card:valueComplexity',
    title: 'Value vs. Complexity',
    kind: 'wide',
    source: SRC.initiatives,
    desc: 'Prioritization matrix — every initiative across programs, color-coded by program',
    render: (d) => (
      <Card title={wt(d, 'card:valueComplexity', 'Value vs. Complexity')}>
        {txn(d, (t) => (
          <ValueComplexityMatrix t={t} />
        ))}
      </Card>
    ),
  },
  {
    id: 'card:initiativesRanked',
    title: 'Initiatives ranked',
    kind: 'wide',
    source: SRC.initiatives,
    desc: 'All initiatives ranked by value and complexity, color-coded by program',
    render: (d) => (
      <Card title={wt(d, 'card:initiativesRanked', 'Initiatives ranked')}>
        {txn(d, (t) => (
          <InitiativesRanked t={t} />
        ))}
      </Card>
    ),
  },
  // Headline count tiles
  tile('tile:divisions', 'Divisions', 'divisions', {
    hint: (t) => `${t.departments} departments`,
    to: '/organization',
    source: SRC.org,
  }),
  tile('tile:roles', 'Roles', 'roles', { to: '/roles', source: SRC.org }),
  tile('tile:valueStreams', 'Value Streams', 'valueStreams', {
    hint: (t) => `${t.domains} domains`,
    to: '/overview',
    source: SRC.vs,
  }),
  ...
  tile('tile:deliverables', 'Deliverables', 'deliverables', {
    to: '/deliverables',
    source: SRC.work,
  }),
  tile('tile:tasks', 'Tasks', 'tasks', { to: '/tasks', source: SRC.work }),
  tile('tile:departments', 'Departments', 'departments', {
    to: '/organization?view=departments',
    source: SRC.org,
  }),
  tile('tile:domains', 'Domains', 'domains', { to: '/overview', source: SRC.vs }),
  tile('tile:applications', 'Applications', 'applications', {
    to: '/applications',
    source: SRC.apps,
  }),
  ...
  tile('tile:processSteps', 'Process Steps', 'processSteps', {
    to: '/overview?view=list',
    source: SRC.vs,
  }),
  ...
  // Cards
  {
    ...
    desc: 'Deeper model counts (sub-processes, I/O, standards…) — pick via its Edit',
    kind: 'card',
    source: SRC.vs,
    render: (d) => {
      const t = d.totals;
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
            {rows.map((s) => (
              <Link
                key={s.label}
                to={s.to}
                className="flex items-center justify-between border-b border-[#f5f5f5] pb-1 group"
              >
  ...
  {
    id: 'card:divisionsByCategory',
    title: 'Divisions by segment',
    desc: 'Division counts by CEO segment (Core Business / IT / Corporate)',
    kind: 'card',
    source: SRC.org,
    render: (d) => (
      <Card
        title={wt(d, 'card:divisionsByCategory', 'Divisions by segment')}
        to="/organization"
        toLabel="Organization"
      >
        <BarList groups={d.divisionsByCategory} color="#4f46e5" />
      </Card>
    ),
  },
  {
    id: 'card:initiativesByStatus',
    title: 'Initiatives by status',
    desc: 'Portfolio initiatives grouped by status',
    kind: 'card',
    source: SRC.initiatives,
    render: (d) => (
      <Card title={wt(d, 'card:initiativesByStatus', 'Initiatives by status')}>
        <BarList groups={d.initiativesByStatus} color="#4f46e5" />
      </Card>
    ),
  },
  {
    id: 'card:initiativesByHealth',
    title: 'Initiatives by health',
    desc: 'Initiatives RAG health (Green / Amber / Red)',
    kind: 'card',
    source: SRC.initiatives,
    render: (d) => (
      <Card title={wt(d, 'card:initiativesByHealth', 'Initiatives by health')}>
        <BarList groups={d.initiativesByHealth} color={HEALTH_COLOR} />
      </Card>
    ),
  },
  {
    id: 'card:risksBySeverity',
    title: 'Risks by severity',
    desc: 'Open risks grouped by severity',
    kind: 'card',
    source: SRC.risks,
    render: (d) => (
      <Card title={wt(d, 'card:risksBySeverity', 'Risks by severity')}>
        <BarList groups={d.risksBySeverity} color={SEVERITY_COLOR} />
      </Card>
    ),
  },
  {
    id: 'card:applicationsByKind',
    title: 'Applications by kind',
    desc: 'The application landscape grouped by kind',
    kind: 'card',
    source: SRC.apps,
    render: (d) => (
      <Card
        title={wt(d, 'card:applicationsByKind', 'Applications by kind')}
        to="/applications"
        toLabel="Applications"
      >
        <BarList groups={d.applicationsByKind} color="#0d9488" />
      </Card>
    ),
  },
  {
    ...
    desc: 'Scenario economics — net impact, benefit, cost',
    kind: 'card',
    source: SRC.scenarios,
    render: (d) => {
      const f = d.financials;
      const rows: [string, number][] = [
        ['Annual net impact', f.annualNetImpact],
        ['Annual benefit', f.annualBenefit],
        ['Annual added cost', f.annualAddedCost],
        ['One-time cost', f.oneTimeCost],
        ['App run cost (TCO)', f.appRunCost],
      ];
      return (
        <Card title={wt(d, 'card:financials', 'Financial impact')} to="/portfolio">
          <div className="space-y-2 text-sm">
            {rows.map(([label, val]) => (
              <div
                key={label}
                className="flex items-center justify-between border-b border-[#f5f5f5] pb-1"
              >
  ...
  {
    id: 'card:topValueStreams',
    title: 'Top value streams',
    desc: 'Value streams ranked by participating roles',
    kind: 'card',
    source: SRC.vs,
    render: (d) => (
  ...
  {
    id: 'card:topDivisions',
    title: 'Top divisions',
    desc: 'Divisions ranked by number of roles',
    kind: 'card',
    source: SRC.org,
    render: (d) => (
...
export const DEFAULT_LAYOUT: string[] = [
  ...
  'tile:tasks',
  'card:modelFootprint',
  'card:valueComplexity',
  'card:initiativesRanked',
];
