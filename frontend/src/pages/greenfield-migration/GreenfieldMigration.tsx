...
export default function ApplicationRationalization({
  ...
  const loadList = useCallback(
    (preferApp?: string) => {
      return api
        .get<StageListItem[]>(`/rationalization${companyId ? `?companyId=${companyId}` : ''}`)
        .then((rows) => {
          setList(rows);
          const apps = [...new Set(rows.map(appOf))];
          setSelectedApp((prev) =>
            preferApp && apps.includes(preferApp)
              ? preferApp
              : prev && apps.includes(prev)
                ? prev
                : (apps[0] ?? null),
          );
        });
    ...
    <div>
      ...
      <div className="flex flex-wrap items-end gap-2 mb-3">
        ...
        <Button
          ...
          className="text-[12px] flex-shrink-0"
          title={`${selectedStage?.name ?? ''} · ${detail ? pct(detail.progress) : ''} migrated`}
        >
          + New…
        </Button>
      </div>
        ...
        <>
          ...
          <div className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-[92vw] max-w-[420px] rounded-xl border border-[#eaeaea] bg-white shadow-xl p-5">
            <h3 className="text-[15px] font-semibold text-[#171717]">
              New application to rationalize
            </h3>
            ...
            <Input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') createInitiative();
              }}
