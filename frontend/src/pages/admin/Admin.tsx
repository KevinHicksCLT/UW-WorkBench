...
export default function Admin() {
  ...
  const section = useMemo(
    () => tab.sections.find((s) => s.key === sectionKey) ?? tab.sections[0],
    [tab, sectionKey],
  );
  ...
  const goTo = (tabK: string, sectionK?: string) => {
    ...
    setTabKey(tabK);
    setSectionKey(
      sectionK && t.sections.some((s) => s.key === sectionK) ? sectionK : t.sections[0].key,
    );
  };
  ...
  return (
    <div>
        ...
        subtitle={
          view === 'dictionary'
            ...
            : view === 'audit'
              ...
              : view === 'signals'
                ? 'The inventory of every trackable signal and metric the operating model can measure.'
      ...
      {/* View switch — Configure, Trackable Metrics, Audit Log, then the Dictionary (D12.2). */}
      ...
      {view === 'dictionary' ? (
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <aside
            className={
              'flex-shrink-0 transition-[width] duration-150 ' +
          ...
          >
            ...
            >
              <svg
                ...
                aria-hidden="true"
                className={
                  'transition-transform duration-150 ' + (navCollapsed ? 'rotate-180' : '')
                }
              >
            ...
            <nav
              className="flex lg:flex-col gap-1 overflow-x-auto lg:overflow-visible -mx-1 px-1 lg:mx-0 lg:px-0"
                  ...
                  >
                    ...
                    {navCollapsed && (
                      <span className="hidden lg:inline">
                        {TAB_SHORT[t.key] ?? t.label.slice(0, 2).toUpperCase()}
                      </span>
                    )}
                  </button>
                );
                  ...
                  >
                    ...
                    {s.hint && (
                      <span
                        className={
                          'ml-1.5 text-[11px] ' +
                          (s.key === sectionKey ? 'text-white/60' : 'text-[#a3a3a3]')
                        }
                      >
                        {s.hint}
                      </span>
