...
function ObjectiveCard({ objective, reload }: { objective: any; reload: () => void }) {
        ...
        <div>
          ...
          {objective.kpis.length === 0 ? (
            ...
            <div className="space-y-2">
              {objective.kpis.map((k: any) => <KpiRow key={k.id} kpi={k} />)}
            </div>
          )}
        </div>
        <div>
          ...
          {objective.initiativeLinks.length === 0 ? (
            ...
            <ul className="text-sm space-y-1.5">
              {objective.initiativeLinks.map((link: any) => (
...
function CreateObjectiveModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
...
function CreateKpiModal({ objectiveId, onClose, onCreated }: { objectiveId: any; onClose: () => void; onCreated: () => void }) {
  const [form, setForm] = useState<any>({
