...
import { fmt, STAGE_ORDER, STAGE_LABELS } from '../lib/format.js';
...
function FinancialsTab({ init, reload }) {
  const [showCreate, setShowCreate] = useState(null); // 'BENEFIT' | 'COST'
...
function LineSection({ title, type, lines, onCreate, onChange }) {
...
function CreateLineModal({ type, initiative, onClose, onCreated }) {
...
function EditValuesModal({ type, line, onClose, onSaved }) {
...
function WorkplanTab({ init, reload }) {
...
function CreateMilestoneModal({ initiativeId, onClose, onCreated }) {
  const [form, setForm] = useState({
    name: '',
...
function CreateRaidModal({ initiativeId, onClose, onCreated }) {
  const [form, setForm] = useState({
...
function AlignmentTab({ init, reload }) {
  ...
  async function remove(objectiveId) {
    await api.delete(`/initiatives/${init.id}/objectives/${objectiveId}`);
    reload();
  }

  const linkedIds = new Set(init.objectiveLinks.map((l) => l.objectiveId));
