  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-3">
        <Input className="sm:max-w-xs" placeholder="Search standards…" value={q} onChange={(e) => setQ(e.target.value)} />
      </div>
                              ...
                              <div className="mt-3">
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {it.valueStreams.map((vs) => (
                                    <Chip as={Link} key={vs.id} to={`/overview?focus=${vs.id}`} className="hover:bg-[#eaeaea]" onClick={(e) => e.stopPropagation()} title={vs.domain ?? ''}>
                                      {vs.name}
                                    </Chip>
                                  ))}
                                </div>
    ...
    <div className="mt-1.5 space-y-1.5">
      ...
      {item.appliers.length > 0 && (
        <div className="flex items-start gap-1.5">
          ...
          <div className="flex flex-wrap gap-1">
            {item.appliers.map((a) => (
              <Chip as={Link} key={a.roleId} to={`/roles/${a.roleId}`} className="hover:bg-[#eaeaea]" onClick={(e) => e.stopPropagation()}>{a.roleName}</Chip>
            ))}
          </div>
        </div>
      )}
