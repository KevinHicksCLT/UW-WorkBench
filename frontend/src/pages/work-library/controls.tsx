...
export function ValueCell({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      api
        .get<{ options: { id: string; name: string; detail?: string | null }[] }>(
          `/work-library/options?kind=${valueKind}&q=${encodeURIComponent(text)}`,
        )
  ...
  if (!entity) {
    // Textarea (auto-sized to the line count) so multi-line procedure values —
    // one sub-step per line, "Done when …" / "Pass when …" last — survive editing.
    return (
      <textarea
        ...
        onBlur={() => {
          if (text !== (current?.value ?? '')) onSave({ value: text || null });
        }}
      />
    );
  }
    ...
    <div ref={boxRef} className="relative">
      ...
      {open && (
        <div className="absolute z-30 left-0 right-0 mt-0.5 max-h-52 overflow-auto rounded-md border border-[#e2e6ea] bg-white shadow-sm text-[12px]">
          ...
          {text.trim() &&
            !options.some((o) => o.name.toLowerCase() === text.trim().toLowerCase()) && (
              <button
                className="block w-full text-left px-2.5 py-1.5 text-[#1d4ed8] border-t border-[#eef1f4] hover:bg-[#f0f6ff]"
                onMouseDown={async (e) => {
                  e.preventDefault();
                  const d = await api.post('/work-library/options', {
                    kind: valueKind,
                    name: text.trim(),
                  });
          ...
          {(current as Answer | null)?.id && valueText(current) && (
            <button
              ...
              onMouseDown={(e) => {
                e.preventDefault();
                onSave({ [fkField]: null, value: null });
                setOpen(false);
              }}
            >
              Clear value
          ...
          {templates.map((t) => {
            ...
            return (
              <button
                ...
                disabled={locked}
                className={
                  'flex w-full items-center gap-2 px-2.5 py-1.5 text-left hover:bg-[#f0f6ff] ' +
                  (locked ? 'opacity-60' : '')
                }
                onClick={() => {
                  if (locked) return;
                  if (multi)
                    onChange(
                      checked ? selectedIds.filter((x) => x !== t.id) : [...selectedIds, t.id],
                    );
                  else {
                    onChange(checked ? [] : [t.id]);
                    setOpen(false);
                  }
                }}
              >
                <span
                  className={
                    ...
                    (checked || locked ? 'text-[#1d4ed8]' : 'text-[#d4d4d4]')
                  }
                >
                  {checked || locked ? '✓' : '○'}
                </span>
                <span className="flex-1">
                  ...
                  <span className="text-[#a3a3a3]">
                    {' '}
                    · {t.keys.length} keys{locked ? ' · always on' : ''}
                  </span>
                </span>
              </button>
            );
