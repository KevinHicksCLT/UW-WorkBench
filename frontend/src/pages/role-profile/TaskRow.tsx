  ...
  return (
    <div className="border-b border-[#f0f0f0] last:border-b-0">
      <div className="flex items-start gap-2.5 px-3 py-2.5">
        {/* Step circle */}
        <span
          className="flex h-6 w-6 items-center justify-center rounded-full text-[11px] font-semibold flex-shrink-0 mt-0.5 bg-[#475569] text-white"
          aria-hidden="true"
        >
          {ordinal}
        </span>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="text-[9px] font-semibold uppercase tracking-[0.12em] text-[#525252] bg-[#f0f1f3] rounded px-1 py-0.5 flex-shrink-0">
                Task
              </span>
            ...
            {task.apps.slice(0, 2).map((a) => (
              <Link
                ...
                className="hidden md:inline-flex items-center rounded-md border border-[#e5e5e5] bg-[#fafafa] px-1.5 py-0.5 text-[11px] text-[#525252] max-w-[150px] truncate hover:border-[#d4d4d4] transition-colors duration-150"
                title={a.name}
              >
                {a.name}
              </Link>
          ...
          {expanded && (
            <div className="mt-2 mb-1 rounded-md border border-[#e2e6ea] bg-[#fbfcfd] overflow-hidden">
              <div className="px-3 py-1.5 border-b border-[#e2e6ea] flex items-center gap-2">
                <span className="text-[9px] font-semibold uppercase tracking-[0.12em] text-[#8a94a0]">
                  Sub-tasks · in order
                </span>
                <span className="text-[10px] text-[#8a94a0] tnum">
              ...
              {task.checklist.length === 0 ? (
                <div className="px-3 py-2 text-[11px] text-[#6b7280]">
                  No sub-tasks are wired to this task yet.
                </div>
              ) : (
                task.checklist.map((s, i) => (
                  <div
                    key={s.id}
                    className="flex items-start gap-2 px-3 py-1.5 border-b border-[#eef1f4] last:border-b-0"
                  >
                    <span className="flex min-w-[18px] h-[18px] items-center justify-center rounded-full bg-[#e5e7eb] text-[#374151] text-[10px] font-semibold mt-px">
                      {i + 1}
                    </span>
