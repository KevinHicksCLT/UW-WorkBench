...
export default function OrgTable() {
  ...
  // from the URL — the param is KEPT so breadcrumbs/back/reload restore it.
  const [searchParams] = useSearchParams();
  const deptOverview = searchParams.get('view') === 'departments';

  const allDivisions = useMemo(
    () => (data ? data.segments.flatMap((s) => s.divisions) : []),
    [data],
  );
  ...
  const allDepartments = useMemo(() => {
    if (!data)
      return [] as (Dept & { divisionId: string; divisionName: string; segment: string })[];
  ...
  const matches = useMemo(
    () =>
      q
        ? allRoles.filter((r) =>
            [r.name, r.roleFamily, r.roleLevel, r.departmentName, r.divisionName, r.segment].some(
  ...
  const dept =
    division && deptId && deptId !== LOOSE
      ? (division.departments.find((d) => d.id === deptId) ?? null)
      : null;
      ...
      {!division ? (
        ...
        <>
          ...
          {q ? (
            ...
            matches.length === 0 ? (
              <Card className="text-sm text-slate-500 italic">
                No roles match “{query.trim()}”.
              </Card>
            ) : (
              <section className="mb-8">
                <div className="text-xs text-[#a3a3a3] mb-3">
                  {matches.length} role{matches.length === 1 ? '' : 's'} matching “{query.trim()}”
                </div>
                <Grid>
                  {matches.map((r) => (
                    <Box
                      ...
                      title={r.name}
                      tag={
                        r.roleLevel && r.roleLevel !== 'Individual Contributor' ? (
                          <Chip>{r.roleLevel}</Chip>
                        ) : undefined
                      }
              ...
              <section key={seg.name} className="mb-8">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs text-[#a3a3a3]">
                    {seg.divisionCount} divisions · {seg.roleCount} roles
                  </span>
                </div>
                <Grid>
        ...
        <>
          <PageHeader
            title={division.name}
            subtitle={`${division.departments.length} teams · ${division.roleCount} roles`}
          />
          {division.departments.length === 0 && division.looseRoles.length === 0 ? (
            <Card className="text-sm text-slate-500 italic">
              No teams or roles in this division.
            </Card>
          ) : (
            <Grid>
              {division.departments.map((dp) => (
                <Box
                  key={dp.id}
                  title={dp.name}
                  meta={`${dp.roleCount} roles`}
                  onClick={() => setDeptId(dp.id)}
                />
              ))}
              {division.looseRoles.length > 0 && (
                <Box
                  title="Direct to division"
                  tag={<Chip>No team</Chip>}
                  meta={`${division.looseRoles.length} roles`}
                  onClick={() => setDeptId(LOOSE)}
                />
              )}
            </Grid>
          )}
        ...
        <>
          <PageHeader
            title={deptId === LOOSE ? 'Direct to division' : (dept?.name ?? '')}
            subtitle={`${rolesInView.length} roles · ${division.name}`}
          />
