// Pipeline Triage (CAP-12, SM-02) — the ranked submission queue. KPI strip up
// top (open count, SLA breach risk, appetite mix, pending proposals), then the
...
// the checkbox multi-selection; row click opens the Risk Workspace (SM-03).
import { useState } from 'react';
import { useApi } from '../../lib/useApi';
import { api } from '../../lib/api';
import { useCompany } from '../../lib/company';
import { useDialogs } from '../../lib/dialogs';
import { Tile, withCompany } from '../../lib/portfolio';
...
export default function PipelineTriage({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  const { data, error, loading, refetch } = useApi<PipelineRow[]>(
    withCompany('/uw/submissions', companyId),
  );
  const { data: pendingActions } = useApi<AgentAction[]>(
    withCompany('/uw/agent-actions?disposition=PENDING', companyId),
  );
  ...
  async function bulkDecline() {
    ...
    setBusy(true);
    try {
      const result = (await api.post(withCompany('/uw/submissions/bulk-decline', companyId), {
  ...
  async function rescore() {
    setBusy(true);
    try {
      const scores = (await api.post(withCompany('/uw/triage/rescore', companyId), {
        submissionIds: [...selected],
      })) as unknown[];
      await dialogs.alert({
  ...
  return (
    <div>
      ...
      <Sheet
        ...
        emptyText="No submissions in the pipeline."
