// Authority Matrix (CAP-08, CAP-13) — authority as data. Grants bind to the TB
...
export default function AuthorityMatrix() {
  ...
  const grants = useApi<AuthorityGrant[]>(withCompany('/uw/authority/grants', companyId));
  const referrals = useApi<ReferralCase[]>(withCompany('/uw/referrals', companyId));
  // Grants bind to the TB Roles catalog — feed the Select from GET /roles
  // ({ rows }, one row per role×responsibility; dedupe by roleId).
  const { data: roleData } = useApi<{ rows: { roleId: string; role: string }[] }>('/roles');
  const roleOptions = useMemo(() => {
    const seen = new Map<string, string>();
    for (const r of roleData?.rows ?? []) if (!seen.has(r.roleId)) seen.set(r.roleId, r.role);
  ...
  async function issue() {
    ...
    setBusy(true);
    try {
      const created = (await api.post(withCompany('/uw/authority/grants', companyId), {
  ...
  return (
    <div>
      ...
      {showForm && (
        <Card className="p-4 mb-4">
          <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-4">
            ...
            <div>
              <Label htmlFor="ag-role">Role (TB Roles catalog)</Label>
              <Select
                id="ag-role"
                value={form.roleId}
      ...
      <Sheet
        ...
        emptyText="No authority grants issued yet."
        summarize={(v) =>
