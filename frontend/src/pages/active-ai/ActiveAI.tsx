...
function Stat({
  ...
  return (
    <Card variant="elevated" className="px-3 py-1.5 flex items-baseline gap-2 flex-wrap">
      <span
        className="text-lg font-semibold tnum leading-snug"
...
export default function ActiveAI() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([adoption, sum]) => {
        setStreams(adoption.valueStreams ?? []);
        setSummary(sum);
      })
  ...
  const rows = useMemo(
    () => streams.map((vs) => ({ vs, cells: vs.cells ?? [0, 0, 0, 0] })),
    [streams],
  );
      ...
      {view === 'signals' ? (
        ...
        <>
          ...
          <Card variant="elevated" className="overflow-hidden mb-2.5">
            <div className="px-4 py-2.5 border-b border-[#eaeaea]">
              ...
              <p className="text-[11px] text-[#666666] mt-0.5">
                Baseline inventory of today's operating model — the orgs, value streams, roles,
                tasks, deliverables and applications a client must understand before planning a
                multi-year AI transformation.
              </p>
            </div>
            ...
            <div className="px-4 py-3 grid grid-cols-3 sm:grid-cols-6 gap-2 border-b border-[#eaeaea]">
              {INVENTORY.map(([key, label]) => (
                <Card
                  key={key}
                  variant="elevated"
                  className="px-3 py-2.5 flex flex-col gap-0.5 items-center text-center"
                >
            ...
            <div className="px-4 py-1.5 flex items-baseline gap-2 flex-wrap border-b border-[#f5f5f5]">
              <h3 className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[#525252]">
                Analysis coverage
              </h3>
              <p
                className="text-[11px] text-[#666666] min-w-0 truncate"
                title="In-progress fill shows analyses underway. Analysis status per value stream / org group / role is edited in Data Admin → Analysis Status."
              >
                How much of the operating model has been assessed for AI opportunity, and whether
                the remaining work is on plan.
              </p>
            </div>
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-[#eaeaea] text-[10px] font-semibold uppercase tracking-[0.08em] text-[#a3a3a3]">
                  ...
                  <th className="text-center px-2 py-1.5">Analyzed</th>
                  <th className="text-left px-2 py-1.5 w-[34%]">Progress</th>
                  <th className="text-center px-2 py-1.5">% complete</th>
                  <th className="text-center px-2 py-1.5">Expected finish</th>
                  <th className="text-center px-4 py-1.5">Plan</th>
                </tr>
              </thead>
              <tbody>
                {(summary?.coverage ?? []).map((c) => (
                  <tr key={c.type} className="border-b border-[#f5f5f5] last:border-0">
                    ...
                    <td className="px-2 py-1 text-sm text-[#525252] tnum whitespace-nowrap text-center">
                      {c.complete}/{c.total}
                    </td>
                    <td className="px-2 py-1">
                      <div
                        className="h-3 rounded bg-[#f5f5f5] overflow-hidden flex"
                        title={`${c.complete} complete · ${c.inProgress} in progress · ${c.notStarted} not started`}
                      >
                        <div
                          style={{
                            width: `${(100 * c.complete) / Math.max(1, c.total)}%`,
                            backgroundColor: '#16a34a',
                          }}
                        />
                        <div
                          style={{
                            width: `${(100 * c.inProgress) / Math.max(1, c.total)}%`,
                            backgroundColor: '#bbf7d0',
                          }}
                        />
                      </div>
                    </td>
                    <td className="px-2 py-1 text-sm text-[#171717] tnum text-center">
                      {c.pctComplete}%
                    </td>
                    <td className="px-2 py-1 text-sm text-[#525252] tnum text-center whitespace-nowrap">
                      {fmtMonth(c.expectedFinish)}
                    </td>
          ...
          <Card variant="elevated" className="overflow-hidden mb-2.5">
            <div className="px-4 py-2.5 border-b border-[#eaeaea] flex items-start justify-between gap-4 flex-wrap">
              <div className="min-w-0">
                ...
                <p
                  ...
                  title="Computed from the canonical Task table (aiDisposition per task) — the same rows as Deliverables & Tasks; edited in Data Admin → Task."
                >
                  Of {summary?.adoption.totalTasks.toLocaleString() ?? 0} tasks, how many were
                  automated, AI-augmented or discarded outright.
                </p>
              </div>
              <div className="flex items-center flex-wrap gap-x-3 gap-y-1">
                {DISPOSITIONS.map((d) => (
                  <span
                    key={d.key}
                    className="inline-flex items-center gap-1.5 text-[10px] text-[#666666]"
                  >
                    <span
            ...
            <div className="px-4 py-2 grid grid-cols-2 sm:grid-cols-4 gap-2 border-b border-[#f5f5f5]">
              <Stat
                ...
                value={`${summary?.adoption.pct.automated ?? 0}%`}
                hint={`${summary?.adoption.counts.automated.toLocaleString()} tasks`}
                color="#15803d"
              />
              <Stat
                label="Tasks discarded"
                value={`${summary?.adoption.pct.discarded ?? 0}%`}
                hint={`${summary?.adoption.counts.discarded.toLocaleString()} eliminated`}
                color="#b45309"
              />
              <Stat
                label="AI augmented"
                value={`${summary?.adoption.pct.augmented ?? 0}%`}
                hint={`${summary?.adoption.counts.augmented.toLocaleString()} tasks`}
              />
              <Stat
                label="Still manual"
                value={`${summary?.adoption.pct.manual ?? 0}%`}
                hint={`${summary?.adoption.counts.manual.toLocaleString()} tasks`}
              />
            </div>
            ...
            <div className="px-4 py-2">
              <div className="grid grid-cols-[minmax(140px,220px)_1fr_auto_auto] gap-x-3 items-center">
                <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[#a3a3a3] py-1">
                  {DIMENSIONS.find(([k]) => k === dimension)?.[1]}
                </div>
                <div />
                <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[#a3a3a3] text-center w-14">
                  Auto
                </div>
                <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-[#a3a3a3] text-center w-14">
                  Disc
                ...
                {breakdown.map((b) => {
                  ...
                  return (
                    <Fragment key={b.name}>
                      <div
                        className="text-xs text-[#171717] truncate py-1"
                        title={`${b.name} · ${b.total} tasks`}
                      >
                        {b.name}
                      </div>
                      <div
                        className="h-4 rounded overflow-hidden flex bg-[#f5f5f5]"
                        title={`${b.total} tasks — ${pct(b.automated)}% automated · ${pct(b.augmented)}% augmented · ${pct(b.discarded)}% discarded · ${pct(b.manual)}% manual`}
                      >
                        {DISPOSITIONS.map((d) => (
                          <div
                            key={d.key}
                            style={{
                              width: `${(100 * b[d.key]) / Math.max(1, b.total)}%`,
                              backgroundColor: d.bg,
                            }}
                          />
                        ))}
                      </div>
                      <div className="text-xs tnum text-center w-14" style={{ color: '#15803d' }}>
                        {pct(b.automated)}%
                      </div>
                      <div className="text-xs tnum text-center w-14" style={{ color: '#b45309' }}>
                        {pct(b.discarded)}%
                      </div>
                    </Fragment>
                  );
              ...
              {breakdown.length === 0 && (
                <EmptyState
                  baseClassName="py-6 text-sm text-[#a3a3a3] italic"
                  message="No assessed tasks yet for this breakdown."
                />
              )}
            </div>
          </Card>
          ...
          <Card variant="elevated" className="overflow-hidden">
            <div className="px-4 py-2.5 border-b border-[#eaeaea] flex items-center justify-between gap-4 flex-wrap">
              <div>
                ...
                <p className="text-[11px] text-[#666666] mt-0.5">
                  How far each value stream has taken AI at every point on the autonomy spectrum —{' '}
                  {heatStats.anyAi}/{heatStats.total} streams with AI in use, {heatStats.autonomous}{' '}
                  running autonomous agents beyond pilot. Click a stream to drill in.
                </p>
              </div>
            ...
            {rows.length === 0 ? (
              <EmptyState
                baseClassName="px-5 py-10 text-sm text-[#a3a3a3] italic"
                message="No value streams defined for this company."
              />
            ) : (
                    ...
                    <tr className="border-b border-[#eaeaea]">
                      ...
                      {MODES.map((m) => (
                        <th
                          ...
                          title={m.desc}
                        >
                          <div className="text-[11px] font-semibold text-[#171717] leading-tight">
                            {m.short}
                          </div>
                          <div className="text-[10px] text-[#a3a3a3] font-normal leading-tight">
                            {m.label}
                          </div>
                        </th>
                      ))}
          ...
          <p className="text-[11px] text-[#a3a3a3] mt-3 italic">
            Adoption levels are read from the operating model (the same value streams as Value
            Streams and Home) and edited in Data Admin → Metrics → AI adoption. Streams with no AI
            yet show “Not used”.
          </p>
        </>
      )}
...
function DomainGroup({
  ...
  domain: string;
  rows: { vs: ValueStream; cells: number[] }[];
}) {
  const [open, setOpen] = useState(true);
  return (
    <>
      <tr
        className="bg-[#fafafa] border-b border-[#eaeaea] cursor-pointer hover:bg-[#f5f5f5]"
        onClick={() => setOpen((v) => !v)}
      >
        <td
          colSpan={MODES.length + 1}
          className="px-5 py-1.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-[#666666] sticky left-0 bg-[#fafafa]"
        >
          <button
            type="button"
            aria-expanded={open}
          ...
          >
            <svg
              ...
              className={
                'text-[#a3a3a3] transition-transform duration-150 ' + (open ? '' : '-rotate-90')
              }
              aria-hidden="true"
            >
              <path d="M6 9l6 6 6-6" />
        ...
        rows.map(({ vs, cells }) => (
          <tr key={vs.id} className="border-b border-[#f5f5f5] hover:bg-[#fafafa] group">
            <td className="px-5 py-2 sticky left-0 bg-white group-hover:bg-[#fafafa] z-10">
              ...
              <Link
                to={`/metrics/${vs.id}`}
                className="text-sm text-[#171717] group-hover:text-[#4338ca] truncate block max-w-[260px]"
              >
                {vs.name}
              </Link>
            </td>
