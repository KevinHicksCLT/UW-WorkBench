...
type Context = {
  role: { id: string; name: string; itemRole: string | null };
  ioItems: CtxRow[];
  processSteps: CtxRow[];
};
  ...
  if (!roleEntity)
    return (
      <Card variant="elevated" className="p-8 text-center text-sm text-[#a3a3a3]">
        Roles aren’t available in this build.
      </Card>
    );
  ...
  return (
    <div>
      <p className="text-sm text-[#666666] mb-4 max-w-2xl">
        Pick a role to edit everything the Role screen shows for it — profile, responsibilities,
        value-stream participation, the deliverables & inputs it owns, and the process steps it
        leads.
      </p>
        ...
        <div className="flex-1 min-w-0">
          {!selected ? (
            <Card variant="elevated" className="p-10 text-center text-sm text-[#a3a3a3]">
              Select a role to edit its full profile.
            </Card>
          ) : (
            <div className="space-y-6">
              <Card className="p-4 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="text-base font-semibold text-[#171717] truncate">
                    {selected.name}
                  </div>
                  {selected.roleLevel && (
                    <div className="text-sm text-[#666666] mt-0.5">
                      {selected.roleLevel}
                      {selected.roleFamily ? ` · ${selected.roleFamily}` : ''}
                    </div>
                  )}
                </div>
                <Button
              ...
              <Section
                ...
                hint="What the role is accountable for — shown grouped on the Role screen."
              >
                {bySlug.get('checklistItem') && (
                  <EntityList
                    entity={bySlug.get('checklistItem')!}
                    companyId={companyId}
                    title="Checklist items"
                    fixed={{ roleId: selected.id }}
                    filter={(r) => r.roleId === selected.id}
                    emptyHint="No checklist items yet — add one."
                    dense
                  />
                )}
                {bySlug.get('roleTask') && (
                  <div className="mt-5">
                    <EntityList
                      entity={bySlug.get('roleTask')!}
                      companyId={companyId}
                      title="Role tasks"
                      fixed={{ roleId: selected.id }}
                      filter={(r) => r.roleId === selected.id}
                      emptyHint="No role tasks yet — add one."
                      dense
                    />
                  </div>
                )}
              ...
              {bySlug.get('roleValueStream') && (
                ...
                >
                  <EntityList
                    ...
                    fixed={{ roleId: selected.id }}
                    filter={(r) => r.roleId === selected.id}
                    emptyHint="No participation links yet — add one."
                    dense
                  />
                </Section>
              )}
      ...
      {bySlug.get('category') && (
        <div className="mt-8 pt-6 border-t border-[#f5f5f5]">
          <Section
            ...
            hint="The shared categories that group role tasks and checklist items across every role."
          >
            <EntityList
              entity={bySlug.get('category')!}
              companyId={companyId}
              title="Categories"
              dense
            />
...
function RoleContextSection({
  ...
  const load = () => {
    ...
    setError('');
    api
      .get<Context>(withCompany(`/admin/role-context/${role.id}`, companyId))
      .then(setCtx)
  ...
  const addDefaults: Record<string, string> =
    kind === 'io' ? { keyRoles: roleAlias, type: 'Deliverable' } : { leads: roleAlias };
  ...
  const remove = async (row: CtxRow) => {
    const ok = await dialogs.confirm({
      ...
      danger: true,
      message:
        'This removes the row from the catalog — it will disappear from every role that referenced it.',
    });
    if (!ok) return;
    try {
  ...
  return (
    <Section title={title} hint={hint}>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#eaeaea] text-left text-[#666666]">
              {kind === 'io' ? (
                ...
                <>
                  <th className="px-3 py-1.5 font-medium text-center">Step</th>
                  <th className="px-3 py-1.5 font-medium">Name</th>
                  <th className="px-3 py-1.5 font-medium">Value stream</th>
                  <th className="px-3 py-1.5 font-medium text-center">Role</th>
                </>
              )}
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={5} className="px-3 py-6 text-center text-[#a3a3a3] italic">
                  None resolve to this role yet — add one below.
                </td>
              </tr>
            ) : (
              rows.map((r) => (
                ...
                >
                  {kind === 'io' ? (
                    ...
                    <>
                      <td className="px-3 py-1.5 tnum text-[#666666] text-center">
                        {r.stepNumber}
                      </td>
                      <td className="px-3 py-1.5 text-[#171717]">{r.name}</td>
                      <td className="px-3 py-1.5 text-[#666666]">{r.valueStreamName ?? '—'}</td>
      ...
      <Button
        variant="secondary"
        className="mt-2"
        onClick={() => setEditing({ create: addDefaults })}
      >
      ...
      {editing !== null && (
        <EntityForm
          ...
          defaults={
            'create' in editing ? (editing as { create: Record<string, string> }).create : undefined
          }
