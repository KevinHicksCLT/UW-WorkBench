...
export default function MetricsSidebar({
  ...
  return (
    <aside
      className="hidden md:flex flex-col bg-white border-l border-[#eaeaea] overflow-y-auto flex-shrink-0 relative z-30"
      style={{
        width: isLeaf ? 460 : 390,
        minWidth: 320,
        ...
        <div className="flex items-center gap-2 mb-1">
          {onBack && (
            <button
              onClick={onBack}
              aria-label="Back"
              className="flex-shrink-0 text-[#525252] hover:text-[#171717] -ml-1"
            >
          ...
          {levelLabel && (
            <div
              className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#a3a3a3]"
              style={accent ? { color: accent } : undefined}
            >
              {levelLabel}
            </div>
          ...
          <button
            ...
            className={
              'ml-auto flex-shrink-0 text-[#a3a3a3] hover:text-[#171717] w-6 h-6 rounded-md hover:bg-[#fafafa] flex items-center justify-center' +
              (onClose ? '' : ' -mr-1')
            }
          >
            <svg
          ...
          {onClose && (
            <button
              onClick={onClose}
              aria-label="Close"
              className="-mr-1 flex-shrink-0 text-[#a3a3a3] hover:text-[#171717] w-6 h-6 rounded-md hover:bg-[#fafafa] flex items-center justify-center"
            >
              <svg
                width="14"
                height="14"
        ...
        <>
          ...
          <div className="p-3 grid grid-cols-2 gap-2 border-b border-[#eaeaea]">
            {dash.tiles.map((t) => {
              ...
              const body = (
                <>
                  <div className="text-[19px] font-bold text-[#171717] leading-none tabular-nums">
                    {t.value === 0 && t.hint ? '—' : fmt(t.value, t.format)}
                  </div>
                  <div className="text-[10.5px] text-[#525252] mt-1 leading-tight flex items-center gap-1">
                    {t.label}
                  </div>
                  {t.hint && (
                    <div className="text-[9px] text-[#a3a3a3] mt-0.5 leading-tight">{t.hint}</div>
                  )}
                </>
              );
              return clickable ? (
                <button
                  key={t.label}
                  onClick={() => onViewAll!(target!)}
                  className="text-left rounded-lg border border-[#eaeaea] bg-[#fafafa] px-3 py-2.5 hover:bg-[#f5f8ff] hover:border-[#dbe7ff] transition-colors duration-150"
                >
                  {body}
                </button>
              ) : (
                <div
                  key={t.label}
                  className="rounded-lg border border-[#eaeaea] bg-[#fafafa] px-3 py-2.5"
                >
                  {body}
                </div>
              );
          ...
          {dash.sections
            .filter((s) => !s.hidden && (s.items.length > 0 || s.emptyText))
            .map((section) => (
