  ...
  return (
    <div
      className={
        ...
        (onClick ? ' cursor-pointer group/cell' : '')
      }
      onClick={
        onClick
          ? (e) => {
...
export default function OrgListExplorer({ focusRoleId = null }: { focusRoleId?: string | null }) {
  ...
  const domainOptions = useMemo(
    () => optionList(flat.filter((r) => matches(r, 'domain')).map((r) => r.domain)),
    selDeps,
  );
  const divisionOptions = useMemo(
    () => optionList(flat.filter((r) => matches(r, 'division')).map((r) => r.divName)),
    selDeps,
  );
  const deptOptions = useMemo(
    () => optionList(flat.filter((r) => matches(r, 'dept')).map((r) => r.deptName)),
    selDeps,
  );
  const roleOptions = useMemo(
    () => optionList(flat.filter((r) => matches(r, 'role')).map((r) => r.roleName)),
    selDeps,
  );
  ...
  const searchMatch = (r: FlatRow) =>
    !needle ||
    [r.domain, r.divName, r.deptName, r.roleName].some((v) => v.toLowerCase().includes(needle));
  const rows = useMemo(() => {
      ...
      <div className="h-full flex relative">
            ...
            {!loading && !error && (
              <>
                <span className="text-[11px] text-[#737373] tnum">
                  {totals.domains} domains · {totals.divisions} divisions · {totals.departments}{' '}
                  departments · {totals.roles} roles
                </span>
                {anyFilter && (
                        ...
                        return (
                          ...
                          >
                            <Cell
                              text={r.domain}
                              accent={hex}
                              onClick={() => openMetrics('domain', r.domain, hex)}
                              dim
                            />
                            <Cell
                              text={r.divName}
                              onClick={() => openMetrics('division', r.divId, hex)}
                            />
                            <Cell
                              text={r.deptName}
                              dim={r.deptId == null}
        ...
        {base && (
          <MetricsSidebar
            ...
            startExpanded
            accent={accentHex}
            onBack={ovStack.length ? onBack : undefined}
