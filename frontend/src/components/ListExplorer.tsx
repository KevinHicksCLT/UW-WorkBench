  ...
  return (
    <div
      className={
        ...
        (onClick ? ' cursor-pointer group/cell' : dead ? ' cursor-default' : '')
      }
      onClick={
        onClick
          ? (e) => {
    ...
    >
      ...
      <span
        className={
          ...
          (onClick ? ' group-hover/cell:underline' : '')
        }
      >
        {text}
      </span>
...
export default function ListExplorer({
  ...
  const flat = useMemo(() => {
    ...
    for (const d of tree?.divisions ?? []) {
      ...
      for (const vs of d.valueStreams) {
        ...
        if (!e) {
          e = { vs, divisions: new Set(), domains: new Set() };
          byId.set(vs.id, e);
        }
  ...
  useEffect(() => {
    ...
    api
      .get<{ valueStreamId: string }>(
        `/explorer/value-stream/${encodeURIComponent(focusVsId)}/focus`,
      )
      .then((f) => {
  ...
  useEffect(() => {
    ...
    for (const r of flat) {
      ...
      if (tokens.some((t) => n === t || (t.length >= 4 && (n.includes(t) || t.includes(n)))))
        names.add(r.vsName);
    }
  ...
  const matches = (r: FlatRow, skip?: Col) =>
    ...
    (skip === 'division' ||
      divisionSel.length === 0 ||
      divisionSel.some((v) => r.divisions.includes(v))) &&
  ...
  const domainOptions = useMemo(
    () => optionList(flat.filter((r) => matches(r, 'domain')).flatMap((r) => r.domains)),
    selDeps,
  );
  const divisionOptions = useMemo(
    () => optionList(flat.filter((r) => matches(r, 'division')).flatMap((r) => r.divisions)),
    selDeps,
  );
  const vsOptions = useMemo(
    () => optionList(flat.filter((r) => matches(r, 'vs')).map((r) => r.vsName)),
    selDeps,
  );
  const subOptions = useMemo(
    () => optionList(flat.filter((r) => matches(r, 'sub')).map((r) => r.areaName)),
    selDeps,
  );
  const stepOptions = useMemo(
    () => optionList(flat.filter((r) => matches(r, 'step')).map((r) => r.stepName)),
    selDeps,
  );
  ...
  useEffect(() => {
    prune(vsSel, vsOptions, setVsSel);
  }, [vsOptions, vsSel]);
  useEffect(() => {
    prune(subSel, subOptions, setSubSel);
  }, [subOptions, subSel]);
  useEffect(() => {
    prune(stepSel, stepOptions, setStepSel);
  }, [stepOptions, stepSel]);
  ...
  const searchMatch = (r: FlatRow) =>
    !needle ||
    [r.domain, r.division, r.vsName, r.areaName, r.stepName].some((v) =>
  ...
  const measure = () => {
    ...
    if (rw)
      setRowsTop(rw.getBoundingClientRect().top - el.getBoundingClientRect().top + el.scrollTop);
  };
  ...
  const end =
    viewH > 0
      ? Math.min(rows.length, Math.ceil((rel + viewH) / rowH) + OVERSCAN)
      : Math.min(rows.length, OVERSCAN * 4);
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          <div ref={scrollRef} onScroll={onScroll} className="flex-1 min-h-0 overflow-auto">
            <div className="px-3 sm:px-4 pb-4">
              {loading ? (
                <LoadingState
                  className="animate-pulse py-8 text-center"
                ...
                <>
                  ...
                  <Card className="p-0">
                    ...
                    <div
                      className={
                        GRID_COLS +
                        ' items-stretch divide-x divide-[#eaeaea] border-b border-[#eaeaea] bg-[#fafafa] rounded-t-lg sticky top-0 z-20'
                      }
                    >
                      <HeaderComboFilter
                        label="Domain"
                    ...
                    <div ref={rowsWrapRef} className="rounded-b-lg overflow-hidden">
                      ...
                      {visible.map((r, vi) => {
                        ...
                        const rowTarget: [string, string] = r.stepId
                          ? ['step', r.stepId]
                          : r.areaId
                            ? ['step', r.areaId]
                            : ['valueStream', r.vsId];
                        ...
                        return (
                          <div
                            ...
                            className={
                              GRID_COLS +
                              ' items-stretch divide-x divide-[#f0f0f0] border-b border-[#f5f5f5] last:border-0 cursor-pointer transition-colors duration-100 ' +
                              (selected ? 'bg-[#f5f8ff] ' : '') +
                              'hover:bg-[#fafafa]'
                            }
                          >
                            <Cell
                              text={r.domain}
                              accent={DOMAIN_HEX[r.domains[0]] ?? '#94a3b8'}
                              dim
                              dead
                            />
                            ...
                            <Cell
                              text={r.vsName}
                              onClick={() => openMetrics('valueStream', r.vsId)}
                            />
                            <Cell
                              text={r.areaName}
                              num={r.areaNum}
                              onClick={r.areaId ? () => openMetrics('step', r.areaId!) : undefined}
                            />
                            <Cell
                              text={r.stepName}
                              num={r.stepNum}
                              onClick={r.stepId ? () => openMetrics('step', r.stepId!) : undefined}
                            />
                          </div>
                        );
                      })}
