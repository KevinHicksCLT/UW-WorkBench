...
// One-off: additively seed the deep levels (initiatives, risks) for every
// company in the demo tenant whose deep levels are still empty. PURELY
// ADDITIVE — passes clear:false so it never deletes; it skips any company that
// already has initiatives, so it is safe to re-run without duplicating rows.
...
async function main() {
  ...
  for (const c of companies) {
    const existing = await prisma.initiative.count({ where: { companyId: c.id } });
    if (existing > 0) {
      console.log(`Skipping ${c.name}: already has ${existing} initiatives (run a full db:seed to rebuild)`);
      continue;
    }
