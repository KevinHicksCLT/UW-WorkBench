...
const STAGES = ['IDEA', 'PLAN', 'EXECUTE', 'REALIZE', 'COMPLETE'];
...
async function main() {
  ...
  for (const [idx, spec] of initiativesData.entries()) {
    ...
    const benefitDatasets = [['ACTUAL', 0.6], ['TARGET', 1.0], ['FORECAST', 0.95]];
    const costDatasets = [['ACTUAL', 0.7], ['TARGET', 1.0], ['FORECAST', 1.05]];
    ...
    const valueScore = { LOW: 1, MEDIUM: 2, HIGH: 3 }[spec.impact];
