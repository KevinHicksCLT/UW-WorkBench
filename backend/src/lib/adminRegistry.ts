...
const DENY = new Set([
  ...
  'ProcessNodeClosure',
  'OrgUnitClosure',
  // Entitlement / preference / provisioning tables — security-sensitive, served
  // only by their bespoke routes (routes/users, routes/me, routes/provisioning).
  'UserValueStream',
  'PermissionSet',
  'PermissionGrant',
...
const GROUPS: { group: string; slugs: string[] }[] = [
  {
    group: 'Organization',
    slugs: ['company', 'orgLevelType', 'orgUnit', 'role', 'roleOrgAssignment'],
  },
  { group: 'Value Streams', slugs: ['processLevelType', 'processNode'] },
  {
    group: 'Work & Control',
    slugs: ['deliverable', 'checklist', 'checklistItem', 'standard', 'testingTemplate'],
  },
  {
    group: 'Connections',
    slugs: [
  ...
  { group: 'Change & Risk', slugs: ['initiative', 'scenario'] },
  {
    ...
    slugs: ['externalParty', 'externalInteraction', 'knowledgeBase', 'agentSkill'],
  },
  {
    group: 'Initiative Tracker (SPM)',
    slugs: [
      'program',
      'workstream',
      'portfolioInitiative',
...
GROUPS.forEach((g, gi) =>
  g.slugs.forEach((slug, wi) => SLUG_POS.set(slug, { group: g.group, gi, wi })),
);
    ...
    if (hasCompanyId) {
      ...
      for (const f of model.fields) {
        if (
          ...
          companyModels.has(f.type)
        ) {
          companyVia = {
            kind: 'relation',
            objectField: f.name,
            scalarField: f.relationFromFields[0],
            targetSlug: camel(f.type),
            targetHasTenantId: tenantModels.has(f.type),
          };
          break;
        }
      }
