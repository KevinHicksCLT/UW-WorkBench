 ...
 */
async function sumMetricValues({ benefitLineId, costLineId, dataset = 'ACTUAL' }) {
  const where = { dataset };
 ...
 */
export async function recomputeInitiative(initiativeId) {
 ...
 */
export async function recomputeWorkstream(workstreamId) {
 ...
 */
export async function summarizeProgram(programId) {
  ...
  const stageDist = initiatives.reduce((acc, i) => {
    acc[i.stage] = (acc[i.stage] || 0) + 1;
    return acc;
  }, {});

  const statusDist = initiatives.reduce((acc, i) => {
