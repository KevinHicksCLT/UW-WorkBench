...
router.get('/division/:id/flow', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    type FlowStep = {
      id: string; step: number; name: string;
      subSteps: { id: string; name: string; step: number; l5: { id: string; name: string; step: number }[] }[];
      inputs: null; outputs: null; upstream: null; downstream: null;
      roles: string[]; categories: string[]; primaryCategory: string | null;
      crossDomain: boolean; unowned: boolean;
    };
