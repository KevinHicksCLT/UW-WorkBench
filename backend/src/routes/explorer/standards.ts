...
router.get('/org-table', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    type RoleEntry = {
      id: string; name: string; roleLevel: null; roleFamily: null; valueStreamCount: number;
      participations: { valueStreamId: string; valueStreamName: string; domain: string | null; participationType: string }[];
    };
    ...
    type DivisionRow = {
      id: string; name: string; segment: string; roleCount: number;
      departments: { id: string; name: string; roles: RoleEntry[]; roleCount: number }[];
      looseRoles: RoleEntry[];
    };
    type SegmentRow = { id: string | null; name: string; divisions: DivisionRow[]; divisionCount: number; roleCount: number };
