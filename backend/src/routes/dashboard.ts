...
const worstStatus = (statuses: string[], fallback: string) =>
  statuses.length
    ? statuses.reduce((a, s) => ((STATUS_SEV[s] ?? 0) > (STATUS_SEV[a] ?? 0) ? s : a), 'ON_TRACK')
    : fallback;
  ...
  try {
    ...
    const [
      ...
      prisma.scenario.aggregate({
        where: w,
        _sum: {
          annualNetImpact: true,
          annualBenefit: true,
          annualAddedCost: true,
          oneTimeCost: true,
        },
      }),
      prisma.application.aggregate({
        where: { companyId, illustrative: false, totalTco: { not: null } },
        _sum: { totalTco: true },
      }),
    ...
    const [
      ...
      (async () => {
        const [ts, mCount] = await Promise.all([
          prisma.telemetrySignal.findMany({
            where: { companyId },
            select: { name: true, isLive: true },
          }),
          prisma.metric.count({ where: w }),
        ]);
        ...
        return (
          mCount +
          ts.filter((s) => s.isLive || (s.name && !liveNames.has(s.name.toLowerCase()))).length
        );
      })(),
    ...
    const [vsNodes, divUnits, nodeRoleGroups, divisionRoleCounts] = await Promise.all([
      ...
      pL2
        ? prisma.processNode.findMany({
            where: { companyId, processLevelTypeId: pL2 },
            select: {
              id: true,
              displayValue: true,
      ...
      oL2
        ? prisma.orgUnit.findMany({
            where: { companyId, orgLevelTypeId: oL2 },
      ...
      oL2
        ? prisma.role.groupBy({
            by: ['orgUnitId'],
            where: { companyId, orgUnitId: { not: null } },
            _count: { _all: true },
          })
        : Promise.resolve([]),
    ]);
    ...
    const [programs, topRisksRaw, openRaidRaw] = await Promise.all([
      ...
      prisma.raidItem.findMany({
        ...
        select: {
          type: true,
          createdAt: true,
          initiative: { select: { workstream: { select: { programId: true } } } },
        },
      }),
    ]);
    ...
    const costSpend = (
      i: (typeof programs)[number]['workstreams'][number]['initiatives'][number],
    ) => {
      const sum = (ds: string) =>
        i.costs.reduce(
          (a, l) => a + l.values.filter((v) => v.dataset === ds).reduce((x, v) => x + v.amount, 0),
          0,
        );
    ...
    const transformation = {
      programs: programs.map((p) => {
        ...
        return {
          ...
          name: p.name,
          status: p.status,
          computedStatus: worstStatus(
            inits.map((i) => i.status),
            p.status,
          ),
          ...
          pctComplete: inits.length
            ? Math.round(
                (inits.reduce((a, i) => a + (STAGE_PROGRESS[i.stage] ?? 0), 0) / inits.length) *
                  100,
              )
            : 0,
          ...
          initiatives: inits.map((i) => ({
            ...
            complexityScore: i.complexityScore,
            ...costSpend(i),
    ...
    const STATUS_LABEL: [string, string][] = [
      ['ON_TRACK', 'On track'],
      ['AT_RISK', 'At risk'],
      ['OFF_TRACK', 'Off track'],
    ];
    const STATUS_HEALTH: [string, string][] = [
      ['Green', 'ON_TRACK'],
      ['Amber', 'AT_RISK'],
      ['Red', 'OFF_TRACK'],
    ];
    const initiativesByStatus = STATUS_LABEL.map(([s, label]) => ({
      key: label,
      count: statusCount(s),
    }));
    const initiativesByHealth = STATUS_HEALTH.map(([health, s]) => ({
      key: health,
      count: statusCount(s),
    }));
    ...
    const layout =
      savedWidgets && !savedWidgets.includes('card:raidSummary')
    ...
    res.json({
      ...
      layout,
      footprintStats:
        (active.dashboardConfig as { footprintStats?: string[] } | null)?.footprintStats ?? null,
      widgetTitles:
        (active.dashboardConfig as { widgetTitles?: Record<string, string> } | null)
          ?.widgetTitles ?? null,
      totals: {
        divisions,
        departments,
        ...
        domains,
        initiatives,
        risks: openRaidByType.find((g) => g.type === 'RISK')?._count._all ?? 0,
        applications,
        metrics,
        scenarios,
        processSteps: steps,
