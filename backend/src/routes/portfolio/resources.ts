/**
 * Initiative resources, per-program resource finance profiles, dependency
 * options/validation, and activities.
 */
...
export function registerResourceActivityRoutes(router: Router): void {
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        logAudit({
          ...
          diff: { resource: data.name, allocationPct: data.allocationPct },
        });
  ...
  async function ownResource(id: string, tenantId: string) {
    const r = await prisma.initiativeResource.findUnique({
      where: { id },
  ...
  router.patch(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const updated = await prisma.initiativeResource.update({
        ...
        logAudit({
          ...
          diff: { resource: existing.name, ...data },
        });
        res.json(updated);
      } catch (e) {
        next(e);
  ...
  router.get('/programs/:id/resources', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Per-person finance profile (roll on/off, type, rate, month overrides) —
      // ProgramResource rows join the assignment aggregation by `name`.
      const [resources, profiles] = await Promise.all([
        prisma.initiativeResource.findMany({
        ...
        prisma.programResource.findMany({
      ...
      res.json(
        [...byName.values()].map((row) => {
          const p = profileByName.get(row.name);
          return {
            ...row,
            overUtilized: row.totalAllocationPct > 100,
            roleType: p?.roleType ?? null,
            rate: p?.rate ?? null,
            rollOn: p?.rollOn ?? null,
            rollOff: p?.rollOff ?? null,
            monthAmounts: (p?.monthAmounts as Record<string, number> | null) ?? {},
          };
        }),
      );
    } catch (e) {
  ...
  // Per-resource finance profile (roll on/off, employment type, monthly rate,
  // month-amount overrides). Upserted by (programId, name) — the same person key
  // the utilization rows aggregate on. `month` sets/clears one override.
  const programResourceSchema = z.object({
    name: z.string().min(1),
    roleType: z.enum(['EMPLOYEE', 'CONTRACTOR']).nullable().optional(),
    rate: z.number().min(0).nullable().optional(),
    rollOn: z.string().nullable().optional(),
    rollOff: z.string().nullable().optional(),
    month: z
      .object({ key: z.string().regex(/^\d{4}-\d{2}$/), amount: z.number().min(0).nullable() })
      .optional(),
  });
  ...
  router.patch(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (!program) return res.status(404).json({ error: 'Not found' });
        const data = programResourceSchema.parse(req.body);

        const existing = await prisma.programResource.findUnique({
          where: { programId_name: { programId: program.id, name: data.name } },
        });
        const patch: Record<string, unknown> = {};
        if ('roleType' in data) patch.roleType = data.roleType ?? null;
        if ('rate' in data) patch.rate = data.rate ?? null;
        if ('rollOn' in data) patch.rollOn = data.rollOn ? new Date(data.rollOn) : null;
        if ('rollOff' in data) patch.rollOff = data.rollOff ? new Date(data.rollOff) : null;
        if (data.month) {
          const months = { ...((existing?.monthAmounts as Record<string, number> | null) ?? {}) };
          if (data.month.amount === null) delete months[data.month.key];
          else months[data.month.key] = data.month.amount;
          patch.monthAmounts = months;
        }

        const saved = existing
          ? await prisma.programResource.update({ where: { id: existing.id }, data: patch })
        ...
        logAudit({
          tenantId: req.tenantId,
          actorEmail: req.user.email,
          entityType: 'ProgramResource',
          entityId: saved.id,
          action: 'RESOURCE_PROFILE_UPDATED',
          diff: { program: program.id, resource: data.name, ...data },
        });
        res.json(saved);
      } catch (e) {
        next(e);
  ...
  const activityCreateSchema = z.object({
    ...
    dependencyType: z
      .enum(['TEAM', 'ROLE', 'PERSON', 'PROJECT', 'CHANGE_APPROVAL'])
      .nullable()
      .optional(),
  ...
  async function validDependency(
    dependsOnId: string,
    initiativeId: string,
    selfId?: string,
  ): Promise<boolean> {
    if (dependsOnId === selfId) return false;
    const dep = await prisma.workplanActivity.findFirst({
      where: { id: dependsOnId, initiativeId },
      select: { id: true },
    });
    return !!dep;
  }
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (data.dependsOnId && !(await validDependency(data.dependsOnId, req.params.id))) {
          return res
            .status(400)
            .json({ error: 'dependsOnId must reference an activity of the same initiative' });
        }
        const last = await prisma.workplanActivity.aggregate({
  ...
  async function ownActivity(id: string, tenantId: string) {
    const a = await prisma.workplanActivity.findUnique({
      where: { id },
      include: { initiative: { select: { tenantId: true } } },
    });
  ...
  router.patch(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (
          data.dependsOnId &&
          !(await validDependency(data.dependsOnId, activity.initiativeId, activity.id))
        ) {
          return res
            .status(400)
            .json({ error: 'dependsOnId must reference an activity of the same initiative' });
        ...
        if (data.startDate) patch.startDate = new Date(data.startDate);
        if (data.endDate) patch.endDate = new Date(data.endDate);
        const updated = await prisma.workplanActivity.update({
        ...
        logAudit({
          ...
          actorEmail: req.user.email,
          entityType: 'PortfolioInitiative',
          entityId: activity.initiativeId,
          action: 'ACTIVITY_UPDATED',
          diff: { activity: activity.name, ...data },
        });
        res.json(updated);
      } catch (e) {
        next(e);
