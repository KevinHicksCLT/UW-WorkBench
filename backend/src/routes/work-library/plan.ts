...
router.get('/plan/:type/:id', async (req, res, next) => {
  try {
    ...
    type EvidenceRow = {
      id: string; kind: string; step: string | null; value: string | null; sortOrder: number;
      standardId?: string; regId?: string;
    ...
    type GovItem = { id: string; name: string; source: string | null; direct: boolean; checklist: EvidenceOut[]; testing: EvidenceOut[] };
    ...
    if (type === 'task') {
      ...
      const evShape = (rows: EvidenceRow[]): EvidenceOut[] => rows.map((r) => ({
...
router.put('/plan/:type/:id/answers', async (req, res, next) => {
  try {
    ...
    for (const row of rows) {
      ...
      if (row.id) {
        const existing = await table.findFirst({ where: { id: row.id, [subjectField]: subject.id } });
        ...
        if (!key) continue;
        const existing = await table.findFirst({ where: { [subjectField]: subject.id, templateKeyId: key.id } });
        if (existing) await table.update({ where: { id: existing.id }, data });
        else await table.create({ data: { companyId: subject.companyId, [subjectField]: subject.id, templateKeyId: key.id, ...data } });
      } else if (row.customKey) {
        await table.create({ data: { companyId: subject.companyId, [subjectField]: subject.id, ...data, customKey: row.customKey } });
      }
    }
...
router.delete('/plan/:type/:id/answers/:answerId', async (req, res, next) => {
  try {
    ...
    const subjectField = type === 'task' ? 'processNodeId' : type === 'standard' ? 'standardId' : 'regId';
    const existing = await table.findFirst({ where: { id: req.params.answerId, [subjectField]: subject.id } });
