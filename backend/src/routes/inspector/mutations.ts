...
export function registerMutationRoutes(router: Router): void {
  ...
  router.post('/:nodeId/roles', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      let created = false;
      if (!roleId) {
        // Home the new role on an OrgUnit (see lib/roleHome.ts — an unhomed role
        // falls into the org views' dead "Unassigned" bucket).
        const orgUnitId = await defaultRoleHome(company.id, node.id);
        const role = await prisma.role.create({
          data: {
            companyId: company.id,
            dbValue: body.newRoleName!,
            displayValue: body.newRoleName!,
            ...(orgUnitId ? { orgUnitId } : {}),
          },
          select: { id: true },
        });
        roleId = role.id;
        created = true;
        await audit(req, 'Role', roleId, 'create', { displayValue: body.newRoleName, orgUnitId });
      } else {
      ...
      await audit(req, 'NodeRole', link.id, 'associate', {
        processNodeId: node.id,
        roleId,
      ...
      res.status(201).json({
        role: {
          nodeRoleId: link.id,
          roleId: link.role.id,
          name: link.role.displayValue,
          relation: link.role_,
          raci: link.ownerLevel,
        },
        created,
        propagation,
      });
  ...
  router.patch('/roles/:nodeRoleId', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const existing = await prisma.nodeRole.findFirst({
        where: { id: req.params.nodeRoleId, companyId: company.id },
        select: { id: true, processNodeId: true, roleId: true, role_: true },
      });
      if (!existing) return res.status(404).json({ error: 'Not found' });
      const body = z
        .object({
          relation: z.enum(['Owner', 'Participant']).optional(),
          raci: z.string().nullable().optional(),
        })
        .parse(req.body);
      ...
      const link = await prisma.nodeRole.update({
        ...
        data: {
          ...(body.relation ? { role_: body.relation } : {}),
          ...(body.raci !== undefined ? { ownerLevel: body.raci } : {}),
        },
        select: {
          id: true,
          role_: true,
  ...
  const appBody = z
    ...
    .refine((b) => b.applicationId || b.newAppName, {
      message: 'applicationId or newAppName required',
    });
    ...
    try {
      ...
      if (!appId) {
        ...
        const owned = await prisma.application.findFirst({
          where: { id: appId, companyId: company.id },
          select: { id: true },
        });
      ...
      await audit(req, 'NodeAppUsage', link.id, 'associate', {
        processNodeId: node.id,
        applicationId: appId,
        usageType: body.usageType,
      });
      res.status(201).json({
        application: {
          usageId: link.id,
          appId: link.application.id,
          name: link.application.name,
          usageType: link.usageType,
        },
        created,
        propagation: { places: ['Applications catalog'], streams: 0 },
      });
    } catch (e) {
      next(e);
    }
  ...
  router.patch(
    '/applications/:usageId',
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const company = await activeCompany(req);
        if (!company) return res.status(404).json({ error: 'No company' });
        const existing = await prisma.nodeAppUsage.findFirst({
          where: { id: req.params.usageId, companyId: company.id },
          select: { id: true, processNodeId: true, applicationId: true, usageType: true },
        });
        if (!existing) return res.status(404).json({ error: 'Not found' });
        const body = z.object({ usageType: z.enum(['performed', 'memorialized']) }).parse(req.body);
        if (body.usageType === existing.usageType) {
          const link = await prisma.nodeAppUsage.findUnique({
            where: { id: existing.id },
            select: {
              id: true,
              usageType: true,
  ...
  async function defaultChecklistId(companyId: string) {
    const found = await prisma.checklist.findFirst({
      where: { companyId, name: 'Process checklist' },
      select: { id: true },
    });
    if (found) return found.id;
    const created = await prisma.checklist.create({
      data: { companyId, name: 'Process checklist' },
      select: { id: true },
    });
    return created.id;
  }
  ...
  router.post('/:nodeId/checklist', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const item = await prisma.checklistItem.create({
        data: { checklistId, text: body.text },
      ...
      await audit(req, 'ChecklistItem', item.id, 'create', {
        processNodeId: node.id,
        text: body.text,
      });
      res
        .status(201)
        .json({ item: { nodeChecklistId: link.id, checklistItemId: item.id, text: item.text } });
    } catch (e) {
      next(e);
    }
  });
  ...
  router.patch(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const link = await prisma.nodeChecklist.findFirst({
          where: { checklistItemId: req.params.checklistItemId, companyId: company.id },
          select: { id: true },
        });
        if (!link) return res.status(404).json({ error: 'Not found' });
        const body = z.object({ text: z.string().min(1) }).parse(req.body);
        const item = await prisma.checklistItem.update({
          where: { id: req.params.checklistItemId },
          data: { text: body.text },
          select: { id: true, text: true },
        });
  ...
  router.delete(
    '/checklist/:nodeChecklistId',
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const company = await activeCompany(req);
        if (!company) return res.status(404).json({ error: 'No company' });
        const link = await prisma.nodeChecklist.findFirst({
          where: { id: req.params.nodeChecklistId, companyId: company.id },
        ...
        const otherLinks = await prisma.nodeChecklist.count({
          where: { checklistItemId: link.checklistItemId },
        });
        if (otherLinks === 0)
          await prisma.checklistItem
            .delete({ where: { id: link.checklistItemId } })
            .catch(() => {});
  ...
  const delivBody = z
    ...
    .refine((b) => b.deliverableId || b.newTitle, {
      message: 'deliverableId or newTitle required',
    });
    ...
    try {
      ...
      if (!deliverableId) {
        const d = await prisma.deliverable.create({
          data: { companyId: company.id, title: body.newTitle! },
          select: { id: true },
        });
        deliverableId = d.id;
        created = true;
        ...
        const owned = await prisma.deliverable.findFirst({
          where: { id: deliverableId, companyId: company.id },
          select: { id: true },
        });
      ...
      const link = await prisma.nodeDeliverable.upsert({
        where: {
          processNodeId_deliverableId: { processNodeId: node.id, deliverableId: deliverableId! },
        },
        update: {},
      ...
      await audit(req, 'NodeDeliverable', link.id, 'associate', {
        processNodeId: node.id,
        deliverableId,
      });
      res
  ...
  router.delete(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const company = await activeCompany(req);
        if (!company) return res.status(404).json({ error: 'No company' });
        const link = await prisma.nodeDeliverable.findFirst({
  ...
  router.put('/:nodeId/testing', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const existing = await prisma.testingTemplate.findFirst({
        where: { taskNodeId: node.id },
        select: { id: true },
      });
      if (existing) {
      ...
      let deliverableId: string | null =
        (
          await prisma.nodeDeliverable.findFirst({
            where: { processNodeId: node.id, companyId: company.id },
            select: { deliverableId: true },
          })
        )?.deliverableId ?? null;
      if (!deliverableId) {
        const d = await prisma.deliverable.create({
          data: { companyId: company.id, title: node.displayValue },
          select: { id: true },
        });
        deliverableId = d.id;
        await prisma.nodeDeliverable.create({
          data: { companyId: company.id, processNodeId: node.id, deliverableId },
        });
        await audit(req, 'Deliverable', deliverableId, 'create', {
