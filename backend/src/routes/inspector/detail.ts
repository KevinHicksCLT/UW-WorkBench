...
router.get('/:nodeId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    type RoleRow = { nodeRoleId?: string; roleId: string; name: string; relation: string; raci?: string | null; tasks?: number };
    let roles: RoleRow[];
    if (detail) {
      roles = nodeRoles
    ...
    type AppRow = { usageId?: string; appId: string; name: string; usageType: string; tasks?: number };
    ...
    type TestingRow = { id: string | null; deliverableId?: string | null; system: string | null; location: string | null; checkType: string | null; expected: string | null };
    ...
    let planRollup: { tasks: number; tasksWithPlan: number; defined: number; total: number } | null = null;
    if (detail) {
