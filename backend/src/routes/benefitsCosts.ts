...
async function tenantInitiative(initiativeId: string, tenantId: string) {
...
router.delete('/lines/:type/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const model: any = type === 'BENEFIT' ? prisma.benefitLine : prisma.costLine;
...
router.post('/values', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const model: any = data.type === 'BENEFIT' ? prisma.benefitLine : prisma.costLine;
    const line = await model.findUnique({
