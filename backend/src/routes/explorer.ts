...
router.get('/overview', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.json({
      ...
      divisions: divisions.map((d) => ({ id: d.id, name: d.name, roles: d._count.roles })),
    });
  } catch (e) { next(e); }
});
...
async function nodeCompany(tenantId: string, id: string) {
  ...
  const c = company.id;
  const [domains, divisions, roleCount, ppl, allVs, apps, risk, inits, kpi] = await Promise.all([
...
async function nodeDivision(tenantId: string, id: string) {
  ...
  return {
    type: 'division', id: div.id, name: div.name, subtitle: `${div.departments.length} departments · ${roles.length} roles · ${ppl.total} people`, illustrative: false,
    lenses: {
...
async function nodeValueStream(tenantId: string, id: string) {
  ...
  const [links, subStreams, apps, metrics, inits, risk, steps, io] = await Promise.all([
    ...
    prisma.subValueStream.findMany({ where: { valueStreamId: vs.id }, orderBy: [{ level: 'asc' }, { name: 'asc' }], select: { id: true, parentId: true, level: true, name: true, inputs: true, outputs: true } }),
  ...
  return {
    ...
    lenses: {
      ...
      how: { processAreas: l3.length, subProcesses: subStreams.filter((s) => s.level === 4).length, steps, initiatives: inits.map((i) => ({ id: i.initiativeId, name: i.initiative.name, health: i.initiative.health })) },
    ...
    children: { childType: 'subValueStream', total: l3.length, nextCursor: null, items: l3.map((s) => child(s.id, 'subValueStream', s.name)) },
  };
}

async function nodeSubValueStream(tenantId: string, id: string) {
  const s = await prisma.subValueStream.findFirst({ where: { id, tenantId }, include: { children: { select: { id: true, name: true } }, valueStream: { select: { id: true } } } });
  if (!s) return null;
  const isL3 = s.level === 3;
  // L3 → child L4s; L4 → process steps for this sub-process
  let steps: any[] = [];
  let io = { Input: 0, Output: 0, Deliverable: 0, total: 0 };
  if (!isL3) {
    [steps, io] = await Promise.all([
      prisma.processStep.findMany({ where: { valueStreamId: s.valueStreamId, l4: s.name }, orderBy: { stepNumber: 'asc' }, select: { id: true, stepNumber: true, name: true, leads: true } }),
      ioCounts([s.valueStreamId], s.name),
    ]);
  } else {
    steps = await prisma.processStep.findMany({ where: { valueStreamId: s.valueStreamId, l3: s.name }, orderBy: { stepNumber: 'asc' }, select: { id: true, stepNumber: true, name: true, leads: true } });
  }
  const items = isL3
    ? s.children.map((c) => child(c.id, 'subValueStream', c.name))
    : steps.map((st) => child(st.id, 'processStep', `${st.stepNumber}. ${st.name}`, { subtitle: st.leads ?? undefined }));
  return {
    type: 'subValueStream', id: s.id, name: s.name, subtitle: isL3 ? 'Process area' : 'Sub-process', illustrative: false,
    lenses: {
      who: {}, what: { output: s.outputs, io }, how: { inputs: s.inputs, outputs: s.outputs, upstream: s.upstream, downstream: s.downstream, steps }, where: {}, why: { notes: s.notes }, howWell: {},
    },
    children: { childType: isL3 ? 'subValueStream' : 'processStep', total: items.length, nextCursor: null, items },
  };
}
...
async function nodeApplication(tenantId: string, id: string) {
  ...
  return {
    ...
    lenses: {
      who: {}, what: { category: app.category }, how: { systemRoles: app.valueStreamLinks.map((l) => ({ stream: l.valueStream.name, role: l.systemRole })) },
      where: { kind: app.kind, vendor: app.vendor, criticality: app.criticality }, why: { criticality: app.criticality }, howWell: { valueStreams: app.valueStreamLinks.length },
    },
    children: { childType: null, total: 0, nextCursor: null, items: [] as any[] },
  };
}
