...
async function companyForReq(req: Request): Promise<string | null> {
  ...
  const company = await prisma.company.findFirst({
    where: { id: cid, tenantId: req.tenantId },
    select: { id: true },
  });
  ...
  try {
    ...
    const [pLevels, oLevels] = await Promise.all([
      prisma.processLevelType.findMany({
        ...
        select: { levelNumber: true, displayValue: true },
      }),
    ...
    const build = (
      spine: Spine,
      levels: { levelNumber: number; displayValue: string }[],
      offset: number,
    ) =>
      levels.map((l, i) => ({
...
router.patch('/types/:key', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const lt = await levelDelegateFor(parsed.spine).findFirst({
      where: { companyId: cid, levelNumber: parsed.level },
      select: { id: true },
    });
    ...
    const updated = await levelDelegateFor(parsed.spine).update({
      where: { id: lt.id },
      data: { displayValue: label.trim() },
    });
    logAudit({
      tenantId: req.tenantId,
      actorEmail: req.user.email,
...
router.get('/tree', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const nodes: FlatNode[] = [
      ...pNodes.map((n) => ({
        id: n.id,
        typeKey: typeKeyFor('processNode', n.processLevelType.levelNumber),
        parentId: n.parentId,
        name: n.displayValue,
        description: attrDesc(n.attributes),
        sortOrder: n.sortOrder,
        provenance: 'real',
        code: n.code ?? null,
        attributes: n.attributes ?? null,
        hidden: isHidden(n.attributes),
        inboundLinks: 0,
      })),
      ...oNodes.map((n) => ({
        id: n.id,
        typeKey: typeKeyFor('orgUnit', n.orgLevelType.levelNumber),
        parentId: n.parentId,
        name: n.displayValue,
        description: null,
        sortOrder: n.sortOrder,
...
async function levelTypeId(spine: Spine, companyId: string, level: number): Promise<string | null> {
  const lt = await levelDelegateFor(spine).findFirst({
    where: { companyId, levelNumber: level },
    select: { id: true },
  });
  return lt?.id ?? null;
}
...
async function planMove(
  ...
  if (newParentId) {
    const parent = await del.findFirst({
      where: { id: newParentId, companyId: cid },
      select: { [levelRel]: { select: { levelNumber: true } } },
    });
    ...
    const inSubtree = await closureDel.findFirst({
      where: { ancestorId: nodeId, descendantId: newParentId },
  ...
  const depths = await closureDel.findMany({
    where: { ancestorId: nodeId },
    select: { depth: true },
  });
  const maxDepth = depths.reduce((m: number, r: { depth: number }) => Math.max(m, r.depth), 0);

  let levels: { id: string; levelNumber: number }[] = await levelDel.findMany({
    where: { companyId: cid },
  ...
  if (neededMax > maxLevel) {
    const toCreate = [];
    for (let n = maxLevel + 1; n <= neededMax; n++)
      toCreate.push({
        companyId: cid,
        levelNumber: n,
        dbValue: `L${n}`,
        displayValue: `Level ${n}`,
      });
    await levelDel.createMany({ data: toCreate, skipDuplicates: true });
    levels = await levelDel.findMany({
      where: { companyId: cid },
      select: { id: true, levelNumber: true },
    });
  }
  ...
  try {
    ...
    if (parentId) {
      const parent = await del.findFirst({
        ...
        select: { id: true, [levelRelationFor(t.spine)]: { select: { levelNumber: true } } },
      });
      ...
      if (parentLevel + 1 !== t.level)
        return res
          .status(400)
          .json({ error: `A ${typeKey} must sit directly under a level-${t.level - 1} node` });
    }
    const ltId = await levelTypeId(t.spine, cid, t.level);
    if (!ltId)
      return res
        .status(400)
        .json({ error: `This company has no level ${t.level} configured for ${t.spine}` });

    const maxSort = await del.aggregate({
      where: { companyId: cid, parentId: parentId ?? null },
      _max: { sortOrder: true },
    });
...
router.patch('/nodes/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (body.parentId !== undefined) {
      ...
      if (spine !== 'orgUnit') {
        ...
        if (newParentId) {
          const parent = await del.findFirst({
            where: { id: newParentId, companyId: cid },
            select: { id: true, [levelRelationFor(spine)]: { select: { levelNumber: true } } },
          });
          if (!parent) return res.status(400).json({ error: 'Parent not found in this company' });
          ...
          for (let i = 0; i < 64 && cursor; i++) {
            ...
            const up: { parentId: string | null } | null = await del.findUnique({
              where: { id: cursor },
    ...
    if (!Object.keys(data).length && moveTo === undefined)
    ...
    audit(req, req.params.id, 'UPDATE', {
      ...data,
      ...(moveTo !== undefined ? { parentId: moveTo } : {}),
    });
    res.json({
      id: updated.id,
      typeKey: typeKeyFor(spine, newLevel),
      parentId: updated.parentId,
      name: updated.displayValue,
      sortOrder: updated.sortOrder,
      attributes: spine === 'processNode' ? updated.attributes : null,
      hidden: isHidden(updated.attributes),
    });
  } catch (e) {
    next(e);
  }
...
router.delete('/nodes/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    invalidateStructureCounts(cid);
    audit(req, req.params.id, 'DELETE', {
      name: found.name,
      typeKey: typeKeyFor(found.spine, found.level),
      children: childCount,
    });
    res.status(204).end();
  } catch (e) {
...
router.post('/nodes/batch', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (ops.length > 500)
      return res.status(400).json({ error: 'Too many ops in one batch (max 500)' });
    ...
    for (const raw of ops) {
      if (raw?.op === 'rename') {
        if (typeof raw.id !== 'string' || typeof raw.name !== 'string' || !raw.name.trim())
          return res.status(400).json({ error: 'rename op needs an id and a non-empty name' });
        ...
        if (typeof raw.id !== 'string')
          return res.status(400).json({ error: 'move op needs an id' });
        const found = await findNode(cid, raw.id);
        if (!found) return res.status(404).json({ error: `Node not found: ${raw.id}` });
        ...
        planned.push({
          ...
          id: raw.id,
          newParentId,
          levelByDepth: plan.levelByDepth,
        });
        ...
        if (!ids.length || ids.some((x) => typeof x !== 'string'))
          return res.status(400).json({ error: 'reorder op needs orderedIds: string[]' });
        ...
        const count = await delegateFor(first.spine).count({
          where: { id: { in: strIds }, companyId: cid },
        });
        if (count !== strIds.length)
          return res.status(404).json({ error: 'reorder references unknown node(s)' });
...
async function findNode(
  companyId: string,
  id: string,
): Promise<{ spine: Spine; level: number; name: string; attributes: unknown } | null> {
  const p = await prisma.processNode.findFirst({
    where: { id, companyId },
    select: {
      displayValue: true,
      attributes: true,
  ...
  if (p)
    return {
      spine: 'processNode',
      level: p.processLevelType.levelNumber,
      name: p.displayValue,
      attributes: p.attributes,
    };
  const o = await prisma.orgUnit.findFirst({
    where: { id, companyId },
  ...
  if (o)
    return {
      spine: 'orgUnit',
      level: o.orgLevelType.levelNumber,
      name: o.displayValue,
      attributes: null,
    };
