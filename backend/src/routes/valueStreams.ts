...
router.get('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const leadOf = (id: string) =>
      taskRoles.get(id)?.find((r) => r.role_ === 'Owner')?.name ?? null;

    const processAreas = l3s.map((area) => ({
      id: area.id,
      name: area.displayValue,
      subProcesses: (byParent.get(area.id) ?? [])
        .filter((s) => s.depth === 2)
        .map((sub) => ({
          id: sub.id,
          ...
          inputs: [] as string[],
          outputs: [] as string[],
          steps: (byParent.get(sub.id) ?? [])
            .filter((s) => s.depth === 3)
            .map((step) => ({ name: step.displayValue, leads: leadOf(step.id) })),
        })),
    }));
    ...
    const roleMap = new Map<
      string,
      { roleId: string; roleName: string; participationType: string }
    >();
