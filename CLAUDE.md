...
Every session, local or cloud, follows this. The **`session-flow`** skill has the exact commands — read it at session start.

1. **Start from a synced `develop`** — `git switch develop && git pull --ff-only origin develop`.
2. **Check develop against production** — a read-only `prisma migrate diff` between the two Neon branches (catalog queries only: no locks, no writes, no branch restore, so it cannot deadlock anything). If production is _ahead_, a change reached `master` without flowing down: stop and tell the user, never auto-repair.
3. **Cut a feature branch before the first edit** — `git switch -c feature/<slug>`, plus a Neon DB branch of the **same name** forked from `develop`, with `backend/.env` repointed at it. (Cloud sessions have no Neon credentials — CI creates the same-name Neon branch on first push; say so.)
4. **Ship every code change before replying** — commit (Conventional Commits, migrations + `erd_v5.mmd` in the same commit) → `git push` → open a **draft PR into `develop`** (`gh pr create --draft --base develop`). This is per turn, not per feature.

Never commit to `develop`/`master` directly, never `--no-verify`, never restore a Neon branch to "sync" databases.
...
The frontend api client prefixes every call with `/api`, and that prefix is **stripped before it reaches Express** in both environments, so **Express mounts routers at the root** (`/roles`, `/work`, `/inspector` — _not_ `/api/roles`):
...
A recurring pattern (see `routes/roles.ts`, `routes/work.ts`): batch-load junctions with a single `{ in: ids }` query, resolve ancestry once via a resolver, then aggregate in memory — **never per-row fan-out**. Standards/regs applicable to an L5 task or a role are _inherited_ from its L2/L3 ancestors (nothing attaches directly to a task); the Tasks list and the Inspector's Governance tab both derive them this way.
...
- **`components/Inspector.tsx` is the unified value-stream sidebar** (shared List + Map tabs) — a tabbed drill-down (Overview / Roles / Applications / Deliverables / Tasks / Checklist / Testing / Governance) over a single `ProcessNode`, fed by `/inspector/:nodeId`. This is _the_ place a node's associations surface.
- Dialogs go through `lib/dialogs` (`useDialogs`) — never `window.confirm/alert/prompt`.

### Database & environments

`DATABASE_URL` (Neon pooled) powers app queries; `DIRECT_URL` (Neon direct) is required by `schema.prisma` for migrations — set both in `backend/.env` (see `.env.example`). The app runs against Neon **branches** (git-flow: `production` / `develop` / feature branches); the active branch is whatever `backend/.env` points at. Branch create/seed/promote/delete has data-loss footguns — use the **`session-flow`** skill (local: authenticated `neonctl`; CI: `scripts/neon-*.mjs` with `NEON_API_KEY`).
