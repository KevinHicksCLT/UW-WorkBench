...
**33 findings** comparing the application against **`IT_Roles_Analytics_v16.xlsx` (the source of truth)** + **`standards_extended/`**, plus the **Initiatives** tab against the **Shibumi blueprint**. Format mirrors `02_backlog.md`: **Evidence/Proof → Why it's wrong → Suggested fix → Approach → Acceptance Criteria (DoD)**. The interactive `gap-review-board.html` is generated from this same set — Accept/Decline there, then hand accepted stories to a coding agent.

> ⚠️ **This list is SEPARATE from yesterday's application-defect backlog (`02_backlog.md` / `review-board.html`, 43 findings), which another agent is already implementing.** Nothing here duplicates that work; these stories come **only** from comparing the app to the spreadsheet (source of truth) and the Shibumi document. Where a symptom also appeared in yesterday's list, it is re-grounded here against the source and noted — triage to whichever backlog the owning agent prefers, but do not double-implement.
...
> Cadence note: a sibling agent is working this repo concurrently (it added `standards_extended/`); coordinate before merging.
...
- **Proof:** Home stat **ROLES 249**; Organization header **240 ROLES**. Same tenant/session (Evidence §A, §B).
- **Why it's wrong:** One entity must return one count everywhere. A 9-role gap means two different queries/filters feed the two screens; users lose trust in headline numbers.
...
- **Acceptance Criteria:** (1) Home == Organization role count for Meridian. (2) Count comes from one shared function. (3) Test asserts equality against the documented definition.

### X2 — Headcount disagrees between Home (743) and Organization (717) 🟠 `PROPOSED`
- **Area:** Cross-tab · **Tabs:** Home, Organization
- **Proof:** Home **743** (Employees 541 + Contingent 202 = 743; Onshore 235 + Nearshore 80 + Offshore 428 = 743 — both internally consistent); Organization **717**. Gap = 26 (Evidence §A, §B).
...
- **Proof:** Value Streams `/overview` header **21**; Telemetry `/active-ai` **29/29** (Evidence §A, §C).
- **Why it's wrong:** "Value stream" is the core object; two inventories means the two tabs read different lists. Against the source (§C) the **29** is correct.
...
- **Proof:** Value Streams tab uses short labels that don't match the workbook (Telemetry does): "Distribution Management" vs **Distribution & Channel Management**; "Actuarial & Reserving" vs **Actuarial Pricing, Reserving & Capital Modeling**; "Reinsurance Management" vs **Reinsurance & Retrocession Management**; etc. (Evidence §C).
...
- **Proof:** App **43 sub-processes / 256 steps**; workbook **L4 Process Master = 131 sub-processes**, **L5 Process Steps = 711 steps** (Evidence §A).
...
- **Proof:** App **240** roles (Org); workbook **159** distinct roles (Role Assignment List / Role_by_Category / Cap–People) (Evidence §A). Divisions (14) and departments (97) match exactly.
- **Why it's wrong:** +81 roles beyond the source with no recorded provenance. Given the standards precedent (§D), the extra roles may be a legitimate enrichment **or** unsourced — that ambiguity itself is the defect.
...
- **Proof:** App **717/743** people; workbook **Cap–People = 159 rows, ΣFTE = 159**, with **no** region (onshore/nearshore/offshore) and **no** employment-type (employee/contingent) fields anywhere in the workbook (keyword scan, Evidence §A). App shows Onshore 235 / Nearshore 80 / Offshore 428 and Employees 541 / Contingent 202.
...
- **Proof:** App **35** applications; workbook **Cap–Application Catalog = 30** (Evidence §A).
...
- **Proof:** Workbook Standards Index **343** (and the per-department detail sheets independently total 343); `standards_extended/` adds **GDPR 21 + CCPA/CPRA 22 + NYDFS-500 22 = 65**, all tagged `area: "Cybersecurity & ISO"`. 48 + 65 = **113** (app Cyber); 343 + 65 = **408** (app total); all other 12 areas match exactly (Evidence §D).
...
- **Proof:** `/portfolio` opens on **Application Rationalization Workspace** (a CAPDAN flow board); Programs is the 2nd rail item (Evidence §E).
...
- **Proof:** App rolls up at program level only (program detail shows $1.8M/$1.3M/$438.2K for Claims Modernization); there is no cross-program EPMO view (Evidence §E). Blueprint specifies an EPMO level aggregating financial/status/schedule/RAID across all programs.
...
- **Proof:** Initiative tabs present = Summary, Financials, Workplan, RAID, Audit. No **Strategic Alignment** tab (Evidence §E). Backend already has `InitiativeObjective` (impact Low/Med/High) and `Kpi`/`StrategicObjective` per `CLAUDE.md`.
...
- **Area:** Initiatives · **Tab:** Initiative detail
- **Proof:** No Charter tab; Summary shows description + details only (Evidence §E). Blueprint Charter = Complexity Score, Value Score, deeper details.
...
- **Area:** Initiatives · **Tab:** Initiative detail
- **Proof:** No Initiative KPIs tab (Evidence §E). Backend has `InitiativeKpi` per `CLAUDE.md`.
...
- **Proof:** No Resources tab on initiative; no capacity/demand anywhere in Initiatives (Evidence §E). Blueprint: Resource pool, Individual Resource, Initiative Resource with capacity-vs-demand and over-utilization.
...
- **Proof:** No scenarios anywhere (Evidence §E). Blueprint: up to 3 scenarios overlaying alternate Start/Due dates and Include/Exclude flags onto baseline initiatives, with "Approve Scenario to Baseline".
...
- **Proof:** Financials shows monthly Benefit/Cost **Actual vs Target** only; no Forecast/Variance series, no Funding Requests (Evidence §E). Backend `MetricValue` already supports a FORECAST dataset per `CLAUDE.md`.
...
- **Proof:** Workplan = a "Milestones" list with Due dates and GATE flags; no activities, dependencies, or timeline (Evidence §E). Blueprint: interactive Gantt with milestone/activity hierarchy, drag-to-reschedule, drag-to-create dependencies.
...
- **Proof:** No presentation/slide-deck/PDF export anywhere in Initiatives (Evidence §E). Blueprint calls auto-refreshing Presentations the "#1 executive deliverable".
...
- **Proof:** Program detail is a single flat workstream→initiative table (Evidence §E). Blueprint Program has 7 tabs: Summary, Initiative Pipeline, Prioritization, Roadmap, RAID, Resources, Scenarios.
...
- **Proof:** Cards = Cumulative Benefit, Cumulative Cost, Net Benefit (Evidence §E). Blueprint Summary = Estimated Annual Benefit, Cumulative Net Benefit, Cumulative Benefit, Cumulative Cost.
...
- **Proof (live):** Program "Claims Modernization" = **At Risk**; both workstreams = **On Track**; workstream "Claims Data Platform" = On Track while its only initiative "Unified Claims Data Platform" = **Off Track, −$185.7K** (Evidence §E).
- **Why it's wrong:** A workstream whose only initiative is Off Track cannot be On Track; health is hand-set and unsynced or the rollup is missing.
...
- **Proof:** Per `CLAUDE.md`, denormalized `cumulative*` recompute only via an explicit recompute call; no write path triggers it (Evidence §E).
- **Why it's wrong:** Editing a benefit/cost line won't update the cumulative figures executives see — stale money on the #1 differentiator.
...
- **Proof:** Workbook **`Items` = 4,743** role-task rows of real, role-specific responsibilities (e.g. *"Set enterprise technology strategy"*, *"Oversee technology standards"*), each tagged Role / Role Family / Category; **`Aligned Role Tasks` = 4,743** maps the same items to Division/Department/Role. The app `/work` shows **5,454 tasks** that are generic repeating templates — *Validate & Review, Approve & Publish, Develop Approach, Define Requirements, Build & Configure* — reused across deliverables (Evidence §E; prior live capture).
...
- **Area:** Deliverables & Tasks parity · **Tab:** Deliverables & Tasks
- **Proof:** Workbook **`Inputs & Outputs Inventory` = 835** rows (**421 inputs**, **414 outputs/deliverables**) keyed to L4 sub-processes with Data Elements and Key Roles. The app shows **441 deliverables** but **no inputs**, and deliverables aren't keyed to the 414 source outputs (Evidence §E).
...
- **Area:** Metrics parity · **Tab:** Telemetry → Trackable Metrics
- **Proof:** Workbook defines a measurement framework: **`Metric_Catalog_Expanded` = 151** (+ `Metric_Catalog` 102 subset) with SourceSystem and RecommendedFactTable, and **`External_Metric_Catalog` = 52** (Jira/ServiceNow/Pega, with calculation patterns). App shows 429 signals (Evidence §E).
...
- **Suggested fix:** Map each app signal to a catalog entry (source system + fact table / calculation); flag any signal with no source.
